-- =============================================================================
-- Forkensics — V2 Migration Acceptance Tests
-- Source: Step 25 Proposal Rev 7 (Section 13)
-- Migration under test: V2__upload_sessions.sql (Codex-approved SHA-256:
--   34c5b40dbfdd829f069d638f0c735ade4ad4a7b7fa5b8ea6d4b895e6e38ab0b3)
--
-- Execution (run after applying V1 + V2 migrations to a fresh local DB):
--   psql "$DATABASE_URL" --set ON_ERROR_STOP=on \
--        -f 08_Migration/tests/V2_acceptance_tests.sql
--
-- All tests run inside a single BEGIN/ROLLBACK — no test data persists.
-- Concurrent race tests (RT-1a through RT-4) require shell harnesses (Section 13.19).
-- =============================================================================
...
BEGIN
  IF EXISTS (
    ...
    RAISE EXCEPTION 'PREFLIGHT FAILED: V2 migration left forkensics_executor→postgres grant';
  END IF;
  RAISE NOTICE 'PREFLIGHT PASSED: V2 migration grants revoked.';
END;
$$;
...
CREATE OR REPLACE FUNCTION test_helpers.make_bare_draft_challenge(p_poster_id uuid, p_group_id uuid)
RETURNS uuid LANGUAGE plpgsql AS $$
DECLARE v_cid uuid;
BEGIN
  PERFORM test_helpers.set_auth_uid(p_poster_id);
  INSERT INTO public.challenges (group_id) VALUES (p_group_id) RETURNING id INTO v_cid;
  INSERT INTO public.challenge_secrets (
    challenge_id, display_dish, canonical_dish, display_restaurant, canonical_restaurant
...
-- Creates a fresh (user, group, challenge) trio — each call makes a new user
-- so multiple calls in one transaction never hit one_active_challenge_per_poster.
...
BEGIN
  ...
  v_gid := test_helpers.make_group(v_uid);
  v_cid := test_helpers.make_bare_draft_challenge(v_uid, v_gid);
...
-- Inserts a session row directly, bypassing the state machine.
-- Owned by forkensics_executor for BYPASSRLS on private schema.
...
DECLARE
  ...
  v_disp text;
BEGIN
  v_orig := 'challenges/' || p_challenge_id::text || '/originals/' || v_sid::text;
  v_disp := 'challenges/' || p_challenge_id::text || '/displays/'  || v_sid::text || '.webp';
...
\echo ''
\echo '============================================================='
\echo 'Forkensics V2 Acceptance Tests'
\echo '============================================================='

-- =============================================================================
-- GROUP 1: SCHEMA VERIFICATION  (Section 13.1)
-- =============================================================================
\echo ''
\echo '--- GROUP 1: Schema Verification ---'
...
DECLARE
  v_idx  text;
  v_idxs text[] := ARRAY[
    'upload_sessions_one_active_per_challenge',
...
DECLARE v_fn text; v_fns text[] := ARRAY[
  ...
  'mark_superseded_media_cleaned','get_media_storage_key','reveal_challenge_service_wrapper',
...
BEGIN
  PERFORM test_helpers.assert(
    EXISTS(SELECT 1 FROM information_schema.triggers
           WHERE event_object_schema='public' AND event_object_table='challenges'
             AND trigger_name='challenge_v2_no_active_upload_on_activate'),
    '1.7a: challenge_v2_no_active_upload_on_activate exists');
  PERFORM test_helpers.assert(
    EXISTS(SELECT 1 FROM information_schema.triggers
           WHERE event_object_schema='public' AND event_object_table='challenges'
             AND trigger_name='challenge_v2_media_ready_on_activate'),
    '1.7b: challenge_v2_media_ready_on_activate exists');
END;
$$;


-- =============================================================================
-- GROUP 2: PARTIAL UNIQUE INDEX  (Section 13.2)
-- =============================================================================
\echo ''
\echo '--- GROUP 2: Partial Unique Index ---'

-- 2.1  Two pending sessions for same challenge → unique violation
DO $$
DECLARE
...
-- 2.2  Pending + complete for same challenge → allowed
DO $$
...
-- 2.3  Failed + pending for same challenge → allowed
DO $$
DECLARE v_uid uuid; v_gid uuid; v_cid uuid; v_s2 uuid;
BEGIN
...
-- =============================================================================
-- GROUP 3: reserve_upload_session  (Section 13.3)
-- =============================================================================
\echo ''
\echo '--- GROUP 3: reserve_upload_session ---'

-- 3.1  Challenge not found → FK_NOT_FOUND
DO $$
...
BEGIN
  ...
  END;
  PERFORM test_helpers.assert(v_caught, '3.1: challenge not found → FK_NOT_FOUND');
END;
$$;
...
BEGIN
  ...
  END;
  PERFORM test_helpers.assert(v_caught, '3.3: non-draft challenge → FK_WRONG_STATE');
END;
$$;
...
BEGIN
  ...
  PERFORM test_helpers.assert(
    v_orig = 'challenges/' || v_cid::text || '/originals/' || v_sid::text,
    '3.6b: original_storage_path format correct');
  PERFORM test_helpers.assert(
    v_disp = 'challenges/' || v_cid::text || '/displays/' || v_sid::text || '.webp',
    '3.6c: display_storage_path format correct');
  PERFORM test_helpers.assert(
...
-- =============================================================================
-- GROUP 4: activate_upload_session  (Section 13.4)
-- =============================================================================
\echo ''
...
BEGIN
  ...
  -- Insert session with short expires_at window
...
BEGIN
  ...
  v_sid := test_helpers.insert_session_direct(v_cid, v_uid, 'pending', 'a4_6',
    now() - interval '1 minute');  -- expires_at in past
  BEGIN
    PERFORM public.activate_upload_session(v_sid, clock_timestamp() + interval '4 minutes');
  EXCEPTION
    WHEN SQLSTATE 'P0001' THEN
...
-- =============================================================================
-- GROUP 5: fail_upload_session  (Section 13.5)
-- =============================================================================
\echo ''
...
BEGIN
  ...
  PERFORM test_helpers.assert(
    EXISTS(SELECT 1 FROM private.upload_sessions WHERE session_id=v_sid AND status='failed' AND failed_reason='FK_TEST_PENDING'),
...
BEGIN
  ...
  PERFORM test_helpers.assert(
    EXISTS(SELECT 1 FROM private.upload_sessions
           WHERE session_id=v_sid AND failed_reason='FK_FIRST_REASON' AND status_changed_at=v_before_ts),
...
-- =============================================================================
-- GROUP 6: advance_upload_session_processing  (Section 13.6)
-- =============================================================================
\echo ''
...
-- =============================================================================
-- GROUP 7: check_upload_session_lease  (Section 13.7)
-- =============================================================================
\echo ''
...
DECLARE v_uid uuid; v_gid uuid; v_cid uuid; v_sid uuid;
BEGIN
  -- 7.1  processing with future lease → true
...
DECLARE v_uid uuid; v_gid uuid; v_cid uuid; v_sid uuid;
BEGIN
  -- 7.2  processing with expired lease → false
...
-- =============================================================================
-- GROUP 8: advance_upload_session_sanitized  (Section 13.8)
-- =============================================================================
\echo ''
...
-- =============================================================================
-- GROUP 9: finalize_upload_session  (Section 13.9)
-- =============================================================================
\echo ''
...
BEGIN
  ...
  PERFORM test_helpers.assert(
    EXISTS(SELECT 1 FROM public.challenges WHERE id=v_cid AND media_object_id=v_mid),
    '9.5e: challenge.media_object_id updated');
  PERFORM test_helpers.assert(
...
-- 9.8  Activate blocked when media=pending_review
DO $$
DECLARE
...
BEGIN
  ...
  SELECT r.media_object_id INTO v_mid FROM public.finalize_upload_session(v_sid, v_hash) r;
  -- media is pending_review; try to activate challenge
  BEGIN
    SET LOCAL ROLE forkensics_executor;
    UPDATE public.challenges SET state='active' WHERE id=v_cid;
    RESET ROLE;
  EXCEPTION WHEN OTHERS THEN
    RESET ROLE; v_caught := true;
  END;
  PERFORM test_helpers.assert(v_caught, '9.8: pending_review media → trigger blocks activation');
END;
$$;

-- 9.9  Activate passes after media=ready
DO $$
DECLARE
  v_uid uuid; v_gid uuid; v_cid uuid; v_sid uuid; v_mid uuid;
...
BEGIN
  ...
  RESET ROLE;
  PERFORM test_helpers.assert(
    EXISTS(SELECT 1 FROM public.challenges WHERE id=v_cid AND state='active'),
    '9.9: media=ready → trigger passes, challenge activated');
END;
$$;


-- =============================================================================
-- GROUP 10: claim_cleanup_sessions  (Section 13.10)
-- =============================================================================
\echo ''
\echo '--- GROUP 10: claim_cleanup_sessions ---'

-- 10.1  Stale pending → expired then claimed
DO $$
...
BEGIN
  ...
  PERFORM test_helpers.assert(v_has_tok, '10.1b: claim_token set');
END;
$$;
...
-- =============================================================================
-- GROUP 11: mark_session_cleaned  (Section 13.11)
-- =============================================================================
\echo ''
...
BEGIN
  ...
  v_sid := test_helpers.insert_session_direct(v_cid, v_uid, 'failed', 'm11_5',
    now()-interval '1 hour', NULL);
  UPDATE private.upload_sessions
  SET cleanup_claim_token=gen_random_uuid(), cleanup_claimed_at=now()-interval '20 minutes',
...
-- =============================================================================
-- GROUP 12: get_upload_capability_expiry  (Section 13.12)
-- =============================================================================
\echo ''
\echo '--- GROUP 12: get_upload_capability_expiry ---'

-- 12.1  Non-NULL future expiry → returns max (+ 30s)
DO $$
...
-- 12.2  NULL storage_upload_expires_at → excluded
DO $$
DECLARE v_uid uuid; v_gid uuid; v_cid uuid; v_blocking timestamptz;
BEGIN
...
-- =============================================================================
-- GROUP 13: quiesce_upload_sessions_for_deletion  (Section 13.13)
-- =============================================================================
\echo ''
...
DECLARE
  ...
  -- Use a dedicated user whose ONLY sessions are these three terminal ones
  v_checker uuid; v_cg uuid; v_cc uuid;
BEGIN
...
-- =============================================================================
-- GROUP 14: get_all_upload_session_paths_for_deletion  (Section 13.14)
-- =============================================================================
\echo ''
...
-- =============================================================================
-- GROUP 15: record_deletion_failure_wrapper  (Section 13.15)
-- =============================================================================
\echo ''
...
BEGIN
  ...
  v_caught := false;
  BEGIN PERFORM public.record_deletion_failure_wrapper(v_uid, 'FK_' || repeat('A', 48));  -- 51 chars
  EXCEPTION WHEN SQLSTATE 'P0001' THEN
    IF SQLERRM LIKE '%FK_INVALID_INPUT%' THEN v_caught := true;
...
-- =============================================================================
-- GROUP 16: V2 TRIGGERS ON CHALLENGES  (Section 13.17)
-- =============================================================================
\echo ''
\echo '--- GROUP 16: V2 Triggers ---'

-- 16.1  Active upload session → upload trigger blocks activation
DO $$
DECLARE v_uid uuid; v_gid uuid; v_cid uuid; v_caught boolean := false;
BEGIN
  ...
  END;
  PERFORM test_helpers.assert(v_caught, '16.1: pending session → trigger blocks activation');
END;
$$;

-- 16.2  complete session no longer blocks (but NULL media still blocks)
DO $$
DECLARE v_uid uuid; v_gid uuid; v_cid uuid; v_sid uuid; v_caught boolean := false;
BEGIN
  ...
  -- Now only media-ready trigger fires (NULL media_object_id → raises)
  BEGIN
    SET LOCAL ROLE forkensics_executor;
    UPDATE public.challenges SET state='active' WHERE id=v_cid;
    RESET ROLE;
  EXCEPTION WHEN OTHERS THEN RESET ROLE; v_caught := true;
  END;
  PERFORM test_helpers.assert(v_caught, '16.2: complete session no longer blocks (media trigger still fires)');
END;
$$;

-- 16.3  NULL media_object_id → media-ready trigger raises
DO $$
DECLARE v_uid uuid; v_gid uuid; v_cid uuid; v_caught boolean := false;
BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN RESET ROLE; v_caught := true;
  END;
  PERFORM test_helpers.assert(v_caught, '16.3: NULL media_object_id → trigger raises');
END;
$$;

-- 16.4–16.7  Non-ready media statuses block activation
DO $$
DECLARE
...
BEGIN
  FOREACH v_status IN ARRAY v_statuses LOOP
    ...
    SET LOCAL ROLE forkensics_executor;
    UPDATE public.challenges SET media_object_id=v_mid WHERE id=v_cid;
    RESET ROLE;
    v_caught := false;
    BEGIN
      SET LOCAL ROLE forkensics_executor;
    ...
    EXCEPTION WHEN OTHERS THEN RESET ROLE; v_caught := true;
    END;
    PERFORM test_helpers.assert(v_caught, '16.4-7: status=' || v_status || ' → trigger raises');
  END LOOP;
END;
$$;

-- 16.8  media=ready → trigger passes
DO $$
...
BEGIN
  ...
  PERFORM test_helpers.assert(
    EXISTS(SELECT 1 FROM public.challenges WHERE id=v_cid AND state='active'),
    '16.8: media=ready → challenge activated');
END;
$$;


-- =============================================================================
-- GROUP 17: PERMISSION VERIFICATION  (Section 13.18)
-- =============================================================================
\echo ''
\echo '--- GROUP 17: Permission Verification ---'
...
DECLARE v_fn text; v_owner name;
  v_fns text[] := ARRAY[
    ...
    'reveal_challenge_service_wrapper','prepare_account_deletion_wrapper','get_deletion_storage_keys',
...
\echo ''
\echo '============================================================='
\echo 'All V2 acceptance tests passed. Rolling back test data.'
\echo '============================================================='
