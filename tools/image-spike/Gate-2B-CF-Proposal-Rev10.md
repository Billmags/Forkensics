...
## Proposal Rev 10 — 2026-08-15

**Supersedes:** Rev 9 (not approved — 2 blockers, Codex SHA-256 `1594e39523c07159d2856470256cad66ebc6a305490c15456bd9b7e4524dac1b`)
...
- **Local creation and editing** of all locked spike artifacts listed in Phase 2.
...
| Party | Status | Note |
|-------|--------|------|
| Claude | ✅ | Rev 10 authored |
| Codex | ✅ | Phase 1 approved |
| Bill | ✅ | Phase 1 approved — SHA-256 `e327cd6d7159386ea4479ce64f33973d5da6d4a69574fdaaca846c21e7e2d435` |

#### Phase 2 — Artifact lock and execution authorization

Requires all of the following before any cloud operation:

**Locked artifacts with SHA-256:**

| Artifact | SHA-256 | Review status |
|----------|---------|---------------|
| `src/index.ts` | `6175c6c12865cc718646647e84767e0391a8c76b183a5443d8b4c5b8ac4e97f2` | ⬜ pending |
| `src/index.test.ts` | `a57bdd73faf0d60e5734ca87ffd344bcfe5546050ace74ad030c518ce358029f` | ⬜ pending |
| `wrangler.toml` | `fdfa1355acc6053819958f7e081c6dbf2804c8bd479dfd190d573305ff55ffac` | ⬜ pending |
| `run-spike.sh` | `61c952d746b33807d30bc18be832af3514ef78b82d1995d20512889f3db4fa9f` | ✅ key_status fix 2026-08-16: `--file /dev/null`→`--pipe >/dev/null`; exit-code-only logic. Run 1 fix: INCONCLUSIVE → conditional continue; `key_wait_absent` + `bucket_wait_absent` retry helpers. Run 2 fix: CF-P-9 retry loop (3 × 15s) for edge propagation; display key names corrected for WebP sources (`.webp.webp`); per-object cleanup verification removed; `bucket_wait_absent` stdout capture fixed. Run 3 fix: CF-P-12 `wrangler delete` suppressed (Wrangler exits non-zero checking KV namespaces — not in token scope, worker does not use KV); worker absence verified via CF API. |
| `fixtures/generate-fixtures.sh` | `b67a6fdeebc56c3222c3bd98ea91dff92e03b14896087d761dfd97c256c3daeb` | ✅ animated WebP fix 2026-08-16: `cwebp`+`webpmux` pipeline replaces `img2webp` (img2webp produced degenerate 122-byte file on macOS with PNG gradient input) |
| `parser/verify-webp.sh` | `e58fa2c9a5cd585041f76fca7d437a55aedd88ff617e34c71a724643e6791524` | ⬜ pending |
| `package.json` | `a95661a1a43cf6724e51a34ac59a57c46168f16771cb609055a1c0895986f134` | ⬜ pending |
| `package-lock.json` | `cb5b65e49dadb443a759c0a329411b6d87200e3f8ed4ab68c59e45de969c5519` | ⬜ pending |
| `tsconfig.json` | `a6b5493cf5d5a06502a711df1ce0eeb2619caf8e0f3533770a5d19ebac1c68c3` | ⬜ pending |
| `vitest.config.ts` | `8d63570e9967eb1160793d561f67f14b3af0866b88a0e3ca5f84c64b8773c913` | ⬜ pending |
| `worker-configuration.d.ts` | `53b36e86b6c9e39d95d8ba1386caf07395423198acf62a08a42a289210e77e0a` | ⬜ pending |

**Pinned tool versions** (recorded at lock time — any change requires re-approval):

| Tool | Pinned version | Record command |
|------|---------------|----------------|
| Node | 24.16.0 | `node --version` |
| npm | 11.13.0 | `npm --version` |
| Wrangler | 4.123.0 | `./node_modules/.bin/wrangler --version` |
| TypeScript | 5.9.3 | `./node_modules/.bin/tsc --version` |
| Vitest | ^4.1.0 (resolved 4.1.10) | from `package.json` `devDependencies` |
| Workers Vitest (`@cloudflare/vitest-pool-workers`) | ^0.21.0 (resolved 0.21.3) | from `package.json` `devDependencies` |

**Static gate evidence (all required before Phase 2 sign-off):**
1. `tsc --noEmit` exits 0, zero errors.
2. Full Vitest suite passes locally.
3. Wrangler `--help` verified for all commands in §4.8.
4. Gitleaks exits 0, zero findings. Two scans required — history and working files — because the runner handles two credentials (`CLOUDFLARE_API_TOKEN` and `SPIKE_SECRET`):
   ```bash
   # Scan Git history
   gitleaks git --no-banner

   # Scan every locked artifact as current files (catches untracked Phase 2 artifacts)
   for TARGET in \
     src \
     fixtures \
     parser \
     ...
     tsconfig.json \
     vitest.config.ts \
     worker-configuration.d.ts; do
     gitleaks dir --no-banner "$TARGET"
   done
   ```
5. Bash syntax validation for all shell artifacts using the same Bash 4+ binary intended for execution:
   ```bash
   BASH_BIN="$(brew --prefix)/bin/bash"
   for SCRIPT in \
     run-spike.sh \
     fixtures/generate-fixtures.sh \
     parser/verify-webp.sh; do
     "$BASH_BIN" -n "$SCRIPT"
   done
   ```
...
| Party | Status | Note |
|-------|--------|------|
| Claude | ✅ | All Phase 2 + Codex rounds 1–5 fixes complete 2026-08-15; `"type":"module"` added; §5.2 mockInfo + §5.5 10 MB test corrected; Vitest ^4.1.0 / @cloudflare/vitest-pool-workers ^0.21.0 / cloudflareTest(); `tsc --noEmit` zero errors, `bash -n` all shell scripts pass; SHAs above are final; CF-P-0b (CPU_LIMIT_MS positive integer) must be provided by Bill before execution |
| Codex | ✅ | Phase 2 artifacts approved 2026-08-15; npm ci clean, 0 vulnerabilities, tsc pass, Vitest 49/49, shell syntax pass, Gitleaks clean, VP8X-only and duplicate-bitstream rejection confirmed, all 11 hashes verified |
| Bill | ✅ | Phase 2 approved 2026-08-15; `CPU_LIMIT_MS=10` verified at Cloudflare dashboard (Free plan, 100,000 req/day cap confirmed) |
...
**Shell requirement:** The runner requires **Bash 4+** for `declare -A`. The preflight guard enforces this. On macOS, the system shell is Bash 3.2 — install Bash 5 via Homebrew and invoke as `/opt/homebrew/bin/bash run-spike.sh`.

**Important (Wrangler 4):** `wrangler r2 object` commands default to local storage. The `--remote` flag is required on every `r2 object put`, `r2 object get`, and `r2 object delete`.
...
#           AND file_size <= 10,485,760 (byte-size gate must NOT trigger)
...
> **Authoritative source:** The SHA-256-locked file `cf-spike/src/index.ts` (see §1.2 Phase 2 lock table, SHA `6175c6c12865cc718646647e84767e0391a8c76b183a5443d8b4c5b8ac4e97f2`) is the single authoritative source for all review purposes.
>
> The embedded listing previously shown here was superseded during the Phase 2 G-fix pass, which added: discriminated-union narrowing for `ImageInfoResponse` (`!("width" in info)` guard before accessing `width`, `height`, `fileSize`); unconditional `fileSize !== head.size` consistency check on the narrowed branch; and the `_ctx: ExecutionContext` third parameter required by `ExportedHandler`. Embedding a copy here would create a second document that diverges on future edits. Read the locked file directly.
...
| **PASS (CF-P-8 INCONCLUSIVE)** | CF-P-8 analytics unavailable, all other probes pass, `REMOTE_CLEANUP_CONFIRMED` | Architecture viable. CF-P-9 200 OK from hosted Worker proves CPU is within the plan limit (Error 1102 would have been returned otherwise). CF-P-8 remains unquantified but is not a blocker. |
...
#### Run 1 — 2026-08-16 (partial)

CF-P-1 through CF-P-9: **all PASS**.

CF-P-8: **INCONCLUSIVE** (Cloudflare metrics dashboard showed "Not enough data" / "0 μs" CPU Time — Free plan analytics lag). Script exited 1 (bug — INCONCLUSIVE should continue). CF-P-10/11/12 did not execute.

Cleanup: Worker confirmed absent. All spike/ object deletes reported "Delete complete" by Wrangler but `key_status` showed "Still present" — R2 propagation delay false positives. Bucket deletion failed as a result. Status: `REMOTE_CLEANUP_REQUIRED`.

**Manual cleanup required before Run 2:** Delete bucket `forkensics-dev-spike` via Cloudflare dashboard (objects are deleted per Wrangler — propagation delay caused false positives; bucket should be empty now).

Fixes applied to `run-spike.sh` (SHA updated above):
- CF-P-8 INCONCLUSIVE: changed from `exit 1` to conditional continue; CF-P-10/11/12 now execute.
- Added `key_wait_absent` (4 attempts, 3s sleep) for R2 post-delete propagation delay.
- Added `bucket_wait_absent` (3 attempts, 3s sleep + retry delete) for bucket deletion.

#### Run 2 — 2026-08-16 (partial)

CF-P-1 through CF-P-7: **all PASS**.

CF-P-9: **FAIL** — hosted worker returned HTTP 404. Root cause: Cloudflare edge propagation delay — `BUCKET.head()` returns null (→ 404) for objects that exist in R2 but aren't yet visible to the freshly-deployed worker. Also confirmed: display key naming bug (WebP sources produce `display/{name}.webp.webp` double extension; cleanup list had single `.webp` — leaving 2 objects in bucket blocking deletion).

Cleanup: Worker confirmed absent. Object deletes reported "Delete complete" by Wrangler but `key_wait_absent` showed false "Still present" (Wrangler `r2 object get --pipe` false-positive for recently-uploaded objects). `bucket_wait_absent` captured Wrangler stdout in return value (missing `>/dev/null`). Bucket not deleted. Status: `REMOTE_CLEANUP_REQUIRED`.

Fixes applied to `run-spike.sh` (SHA updated above):
- CF-P-9: retry loop (3 attempts × 15s sleep) for edge propagation.
- Display key names: `display/fixture-static-icc-xmp.webp.webp` and `display/fixture-animated.webp.webp` (double extension matches `display/${key.split("/").pop()!}.webp` in `src/index.ts` line 119).
- Cleanup: per-object verification removed; `bucket_wait_absent` is now the authoritative check.
- `bucket_wait_absent`: added `>/dev/null` to wrangler delete calls; increased to 5 attempts × 5s sleep.

**Manual cleanup required before Run 3:** Delete bucket `forkensics-dev-spike` via Cloudflare R2 dashboard (empty bucket first if needed — two `.webp.webp` display objects may remain).

#### Run 3 — 2026-08-16 — **COMPLETE**

| Probe | Result | Notes |
|-------|--------|-------|
| CF-P-1 | ✅ PASS | Bucket created, public access disabled |
| CF-P-2 | ✅ PASS | worker-configuration.d.ts SHA verified; tsc zero errors |
| CF-P-3 | ✅ PASS | All 5 fixtures generated + uploaded |
| CF-P-4-A | ✅ PASS | Oversized bytes → 422, no display write |
| CF-P-4-B | ✅ PASS | Oversized pixels → 422, no display write |
| CF-P-4-C | ✅ PASS | JPEG → 200, WebP output |
| CF-P-4-D | ✅ PASS | Static WebP → 200, WebP output |
| CF-P-4-E | ✅ PASS | Animated WebP → 200, non-animated WebP output |
| CF-P-5 | ✅ PASS | No prohibited metadata (EXIF/XMP/ICC/GPS) in any output |
| CF-P-6 | ✅ PASS | All outputs are valid structural WebP; no animation flag |
| CF-P-7 | ✅ PASS | SHA-256 verified for all 3 outputs |
| CF-P-9 | ✅ PASS | Hosted worker deployed; JPEG transform confirmed (attempt 2 after 15s edge propagation) |
| CF-P-8 | ⚠️ INCONCLUSIVE | Free plan analytics lag — "Not enough data" in dashboard; CF-P-9 200 OK proves CPU within budget |
| CF-P-10 | ✅ PASS | 401 for no-auth and wrong-token requests |
| CF-P-11 | ✅ PASS | display/fixture-exif.jpg.webp present in R2; SHA matches CF-P-9 output |
| CF-P-12 | ✅ PASS (note) | Worker deleted (Wrangler exits non-zero due to KV check — not in token scope — but CF API confirms absent); all objects deleted; bucket deleted |
| Cleanup | ✅ REMOTE_CLEANUP_CONFIRMED | All spike resources removed |

**Overall verdict: PASS (CF-P-8 INCONCLUSIVE) — Architecture viable. Cloudflare R2 + Images Binding confirmed as a viable production path.**
...
| AH-1–4 | Stream, gitignore, grep, Vitest | `head()`, `git check-ignore`, CF API, expanded matrix |

### Rev 6 → Rev 7

| # | Blocker | Resolution |
|---|---------|------------|
| C-1 | `.targets[0].url` wrong shape | Changed to `.targets[0]` |
| C-2 | ETag race tests wrong after `head()` | §5.1/5.2 mock sequences corrected; call-count assertions |
| C-3 | `3101×5000 = 15,505,000` | `3100×5000 = 15,500,000` (pass); `2739×5659 = 15,500,001` (fail) |
| C-4 | `case "$(fn)"` swallows failures | Helpers return 0/1; callers capture + check + default branch |
| C-5 | Flag too late; missing display keys | `BUCKET_MUTATION_ATTEMPTED=true` before create; oversized display keys added |
| C-6 | `grep "enabled"` matches "not enabled" | CF API `/domains/managed` with exact `case` |
| M-1–3 | umask, final sentence, Claude ✅ | Applied |

### Rev 7 → Rev 8

| # | Blocker | Resolution |
|---|---------|------------|
| D-1 | `--remote` missing from all `r2 object` commands | Added throughout |
| D-2 | Wrong dev-URL endpoint | `/r2/buckets/$BUCKET/domains/managed` |
| D-3 | Pixel fixture byte-size not asserted | `PIXEL_SIZE <= 10,485,760` added to `generate-fixtures.sh` |
| D-4 | `assert_exact_ct` accepts supersets; size ceiling missing; Test E | Exact strip+normalize; ceiling in `assert_size_header`; Test E calls helper |
| H-1 | `worker_api_status` stores body | `-o /dev/null -w "%{http_code}"` |

### Rev 8 → Rev 9

| # | Blocker | Resolution |
|---|---------|------------|
| E-1 | No Bash version guard; `declare -A` fails on macOS Bash 3.2 | `BASH_VERSINFO[0] >= 4` check in preflight with macOS install guidance; guard runs before any `declare -A` usage |
| E-2 | Phase 2 artifact lock incomplete | Added `package.json`, `package-lock.json`, `tsconfig.json`, `vitest.config.ts`, `worker-configuration.d.ts` to SHA-256 table; added pinned version table (Node, npm, Wrangler, TypeScript, Vitest, Workers Vitest); added `gitleaks git --no-banner` to static gate |
| E-3 | Cloudflare API calls unbounded | `--connect-timeout 5 --max-time 15` added to all CF API curl calls: `worker_api_status`, `bucket_api_status`, account verification, CF-P-1 managed-domain check |

### Rev 9 → Rev 10

| # | Blocker | Resolution |
|---|---------|------------|
| F-1 | `gitleaks git` does not scan untracked Phase 2 artifacts | Added `gitleaks dir --no-banner "$TARGET"` loop over every locked artifact path after the history scan; both scans required |
| F-2 | No shell syntax gate for shell artifacts | Added `"$BASH_BIN" -n "$SCRIPT"` loop over `run-spike.sh`, `fixtures/generate-fixtures.sh`, `parser/verify-webp.sh` using `BASH_BIN="$(brew --prefix)/bin/bash"` (same binary as the run) |

---

*Proposal Rev 10 — 2026-08-15 — awaiting Codex and Bill Phase 1 sign-off*
