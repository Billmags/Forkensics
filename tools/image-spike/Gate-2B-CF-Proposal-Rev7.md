...
## Proposal Rev 7 — 2026-08-15

**Supersedes:** Rev 6 (not approved — 6 blockers, Codex SHA-256 `c6ba17dd67e71f92f67205fc5003a4ff13b79c6d6b59adb808e27e28cdc1b81d`)
...
### §4.7 Deploy URL Extraction via `WRANGLER_OUTPUT_FILE_PATH` (fix: `.targets[0]`)
...
# targets[0] is the URL string directly — no .url sub-field
...
| `wrangler whoami` | — |
| CF API `GET /r2/buckets/{name}/dev-url` | Verify endpoint and `result.enabled` field shape |
...
# Set BEFORE (or unconditionally immediately after) each mutation attempt
# so cleanup runs even on partial failure.
...
# "error" is output on operational failure; message goes to stderr.
# Return code: 0 for exists/absent, 1 for error.
# Call sites: capture to variable, check return code with ||, then case with default.
# Never use case "$(fn)" directly — exit codes inside $() are swallowed.

key_status() {
  local key="$1" err_tmp
  err_tmp=$(mktemp)
  if "$WRANGLER" r2 object get "$BUCKET/$key" --file /dev/null >"$err_tmp" 2>&1; then
...
worker_api_status() {
  ...
  case "$http_code" in
    ...
    *)   printf 'FAIL: worker_api_status HTTP %s: %s\n' "$http_code" "$body" >&2
         echo "error"; return 1 ;;
  esac
}
...
# Verify public access disabled via CF API (exact enabled/disabled/error — no grep)
# Endpoint: GET /accounts/{id}/r2/buckets/{name}/dev-url
# Must be confirmed against CF docs during Phase 1 verification; if endpoint differs, update here.
DEV_URL_RESP=$(curl -s -H "Authorization: Bearer $CLOUDFLARE_API_TOKEN" \
  "https://api.cloudflare.com/client/v4/accounts/$CF_ACCOUNT_ID/r2/buckets/$BUCKET/dev-url")
DEV_URL_ENABLED=$(printf '%s\n' "$DEV_URL_RESP" | jq -r '.result.enabled // "error"')
case "$DEV_URL_ENABLED" in
  false) echo "PASS CF-P-1: Bucket created, dev URL confirmed disabled" ;;
  true)  echo "FAIL CF-P-1: Public dev URL is enabled"; exit 1 ;;
  *)     echo "FAIL CF-P-1: dev-url API unexpected response: $DEV_URL_RESP"; exit 1 ;;
esac
```
...
#    b. PROVES each fixture meets its preconditions via exiftool + file-size checks:
...
#       - fixture-oversized-px.jpg: width × height > 15,500,000 per exiftool
#       - fixture-oversized.jpg: wc -c > 10,485,760
...
| `spike/fixture-oversized-px.jpg` | ≤ 10 MB but > 15.5 MP (pixel gate) | width × height > 15,500,000 per exiftool |
...
# Atomic creation at mode 0600 before writing any secret (umask avoids brief wide-open window)
...
assert_exact_ct() {
  local headers="$1" label="$2"
  local ct; ct=$(grep -i "^content-type:" "$headers" | head -1 | tr -d '\r')
  [[ "$ct" == *"image/webp"* ]] \
    || { echo "FAIL $label: Content-Type not image/webp: $ct"; exit 1; }
}
assert_size_header() {
  local headers="$1" file="$2" label="$3"
  local hdr_size file_size
  hdr_size=$(grep -i "^x-forkensics-size:" "$headers" | head -1 | awk '{print $2}' | tr -d '[:space:]')
...
STATUS=$(key_status "display/fixture-oversized.jpg.webp") || { echo "FAIL CF-P-4-A: key_status error"; exit 1; }
...
STATUS=$(key_status "display/fixture-oversized-px.jpg.webp") || { echo "FAIL CF-P-4-B: key_status error"; exit 1; }
...
# Set flag before deploy attempt (B-6 pattern)
SECRET_OR_WORKER_CREATED=true
WRANGLER_OUTPUT_FILE_PATH="$WRANGLER_OUT" \
  "$WRANGLER" deploy --secrets-file "$SECRETS_TMP"
WORKER_DEPLOYED=true
...
# Extract Worker URL — targets[0] is a URL string directly (§4.7)
WORKER_URL=$(jq -r 'select(.type == "deploy") | .targets[0] // empty' "$WRANGLER_OUT" \
  | head -1)
rm -f "$WRANGLER_OUT"
[ -n "$WORKER_URL" ] \
...
CF-P-8 pauses for the operator to record CPU analytics from the Cloudflare Dashboard and enforces the threshold before continuing. Missing, delayed, or ambiguous analytics produce INCONCLUSIVE → FAIL.
...
```typescript
// src/index.ts — forkensics-image-spike Rev 7 — SPIKE ONLY
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // based on wrangler types output. Never use `as` to add a field.
...
| # | Blocker | Resolution |
|---|---------|------------|
| C-1 | `.targets[0].url` wrong shape | Changed to `.targets[0]` (URL string directly, per Cloudflare docs); §4.7 and CF-P-9 updated |
| C-2 | ETag race tests wrong after `head()` | §5.1 first `get()` returns metadata-only (Read 2 miss); §5.2 first `get()` returns body, second returns metadata-only (Read 3 miss); call count assertions added |
| C-3 | Pixel boundary dimensions wrong (`3101×5000 = 15,505,000`) | Changed to exact dimensions: pass = `3100×5000 = 15,500,000`; fail = `2739×5659 = 15,500,001` |
| C-4 | Cleanup swallows verifier failures via `case "$(fn)"` | All helpers return 0/1 and output "error" string; all call sites capture to variable with `||` check and `case` with explicit default branch |
| C-5 | `BUCKET_CREATED=true` set only after success; display keys missing | `BUCKET_MUTATION_ATTEMPTED=true` set before create; `display/fixture-oversized.jpg.webp` and `display/fixture-oversized-px.jpg.webp` added to EXIT cleanup and CF-P-12 |
| C-6 | Dev-URL check grep `enabled` can match "not enabled" | CF API `GET /r2/buckets/{name}/dev-url` with explicit `case` on `result.enabled` true/false/error |
| M-1 | `.dev.vars` brief wide-open mode after write | `(umask 077; printf ... > .dev.vars)` in CF-P-4 and CF-P-9 |
| M-2 | Final sentence said "Bill must insert account ID" | Removed — account ID already filled |
| M-3 | Phase 1 Claude row showed ⬜ pending | Updated to ✅ |

---

*Proposal Rev 7 — 2026-08-15 — awaiting Codex and Bill Phase 1 sign-off*
