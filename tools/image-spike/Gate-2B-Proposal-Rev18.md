# Gate 2B Proposal — Rev 18
# Managed Supabase Image Transformation Feasibility Spike

**Date:** 2026-08-15  
**Status:** DRAFT — awaiting Codex review and three-party approval  
**Predecessor:** Gate-2B-Proposal-Rev17.md (NOT APPROVED — six blockers)  
**Authorized project:** `hkfrbdpedrxmbsawnbpr` (forkensics-dev only)

**Rev 18 blockers closed from Rev 17:**

| # | Blocker | Section changed |
|---|---------|----------------|
| B-1 | Runner JWT preflight added: five checks without printing the key; three rejection scenarios (no credentials, anon JWT, authenticated-user JWT); `sb_secret_` key incompatibility noted | §3.7, §3.8 |
| B-2 | P-1 anon test sends both `Authorization` and `apikey` headers; fallback text corrected to "anon credentials can access the unsigned URL" | P-1 |
| B-3 | P-3 `Comment` removed as ExifTool group; replaced with explicit prohibited tag names (`Comment`, `UserComment`, `XPComment`); VP8X comment chunk claim removed | P-3 |
| B-4 | P-6 RIFF traversal endpoint corrected from `bytes[12..12+riff_size)` to `bytes[12..declared_total)`; zero-valued padding required; unknown chunks rejected; VP8X feature flag validation added; strict chunk allowlist for still images | P-6 |
| B-5 | P-8 verdict tiers corrected: <100 MB = PASS; 100–<200 MB = INCONCLUSIVE; ≥200 MB or 546 = FAIL | P-8 |
| B-6 | P-11 retry logic corrected: 404 = PASS; 200 = retry up to limit then FAIL; 401/403/429/5xx = immediate FAIL; service-role list query added as independent confirmation | P-11 |
...
This is a **feasibility spike**, not an implementation. Each requirement must be proved independently before a Rev 19 implementation proposal is drafted.
...
- Upload of test fixtures to `game-media` under `gate2b-r18/<run-id>/`
- Execution of the spike protocol (P-1 through P-14)
- Deletion of all spike objects and the deployed function per §3.6
...
- **Object prefix:** `gate2b-r18/<run-id>/` where `<run-id>` is a UUID generated at spike start
...
2. **List** all objects under `gate2b-r18/<run-id>/` in `game-media`, paginating all result pages.
3. **Delete** each enumerated object; confirm each deletion individually.
4. **Re-list** the prefix; require exactly zero objects remaining. Any remaining objects → log cleanup failure, exit non-zero.
5. Delete DB test fixtures if created; confirm deletion.
6. Preserve evidence directory (`gate2b-evidence-r18-<YYYYMMDDTHHMMSSZ>/`) before exit.
...
**Callers that must be rejected:**
- No `Authorization` header → gateway returns 401 before handler
- Anon JWT (`role=anon`) → handler returns 403
- Authenticated-user JWT (`role=authenticated`) → handler returns 403
- Any JWT with `role ≠ service_role` → handler returns 403

**Key format note:** `verify_jwt = true` requires a JWT-format service-role key (three dot-separated base64url segments). Newer Supabase `sb_secret_...` format keys are not JWTs; they will fail the gateway signature check. The runner preflight (§3.8) detects this before deployment.

P-1 must assert that anon-JWT and authenticated-user-JWT requests return 403.

### 3.8 Runner JWT Preflight (Pre-Deployment)

Before deploying the spike function, the runner script must verify the service-role key without printing it. The following checks must all pass; any failure aborts the run before deployment:

1. **Project ref:** `PROJECT_REF` environment variable equals exactly `hkfrbdpedrxmbsawnbpr`. Print `✓ PROJECT_REF` on pass; print `FAIL: unexpected PROJECT_REF` and exit 1 on mismatch.
2. **Three JWT segments:** Split `SERVICE_ROLE_KEY` on `.`; require exactly three non-empty segments. Print `✓ key has 3 segments` on pass; print `FAIL: key does not appear to be a JWT (wrong segment count)` and exit 1 on failure. This also catches `sb_secret_...` keys.
3. **Payload decodes:** Base64url-decode the middle segment and parse as JSON. Print `✓ payload decodes` on pass; print `FAIL: payload not decodable` and exit 1 on failure. Never print the decoded payload.
4. **`role` claim:** Read the `role` field from the decoded payload. Require `role == "service_role"`. Print `✓ role=service_role` on pass; print `FAIL: role is not service_role` and exit 1 on mismatch. Never print the actual role value in a failure message.
5. **Not expired:** Read the `exp` field (Unix epoch seconds). Require `exp > $(date +%s)`. Print `✓ key not expired` on pass; print `FAIL: key is expired` and exit 1 on failure. Never print the exp value.
...
- Read first chunk FourCC at byte 12:
  ...
  - `VP8X`: read flags (4 bytes); animation flag = bit 1 (0x02) of flags byte 0. Extract canvas width and height. Reject if animation flag is set.
...
3. If not included: document plan required, monthly cost, per-transformation cost. Stop — no artifact drafting until separate upgrade approval obtained.
...
**Objective:** Confirm private-bucket transformation succeeds with administrative authorization and the object is inaccessible without it.

**Fixture:** F-SMALL, uploaded to `game-media` under the run prefix.

**Method:**
1. Using the service-role key, generate a **short-lived signed transformation URL** for F-SMALL with `Accept: image/webp`.
2. Fetch the signed URL with `Accept: image/webp`. Record HTTP status and response body.
3. Confirm the equivalent **unsigned** storage URL (no signing parameters) returns non-200 when fetched with no headers.
4. Confirm the unsigned URL returns non-200 when fetched with both `Authorization: Bearer <anon-jwt>` and `apikey: <anon-key>` headers present.
5. Send a request to the spike function with an **anon JWT** in the `Authorization` header; confirm HTTP 403.
6. Send a request to the spike function with an **authenticated-user JWT** (`role=authenticated`) in the `Authorization` header; confirm HTTP 403.

**Assertions:**
- Signed URL, `Accept: image/webp`: HTTP 200, non-empty body
- Unsigned URL, no headers: non-200
- Unsigned URL, both anon headers present: non-200
...
| Comments | — | `Comment`, `UserComment`, `XPComment` | (none; VP8X does not contain comment text) |
...
- Structural ExifTool properties (FileType, ImageWidth, ImageHeight, MIMEType, FileSize, Megapixels) are not prohibited and their presence does not fail this assertion
- No prohibited chunk IDs appear in the chunk map (`EXIF`, `ICCP`, `XMP `, `ANIM`, `ANMF`, or any non-allowlisted FourCC — see P-6 allowlist)
- Source-fixture preflight confirmed expected metadata was present before transformation
...
|-------------|-----------------|
| F-MALFORMED (truncated JPEG) | imgproxy must return non-200. Any 200 response fails this test. |
| F-ANIMATED (animated WebP) | Spike function detects VP8X animation flag via §7 header parser and returns rejection **without issuing any transformation request**. Confirmed by `transform_fetch_attempted=false`. |
| F-POLYGLOT (JPEG + appended ZIP) | Accepted **only if** all conditions of the structural validation below are satisfied. Any condition failure → reject. |

**WebP structural validation for polyglot acceptance:**

Definitions:
- `riff_size` = uint32 LE value at bytes 4–7
...
3. Parse all chunks within `bytes[12 .. declared_total)`:
   - Each chunk: 4-byte FourCC + 4-byte little-endian chunk payload size + payload bytes + 1 padding byte if payload size is odd.
...
4. Verify `declared_total` equals end of the last chunk (including any padding). No bytes may follow.
5. **VP8X feature flag validation** (if a `VP8X` chunk is present): read the 4-byte flags field at VP8X payload offset 0. The following flag bits must be 0; any set bit → reject:
   - Bit 1 (0x02): Animation flag
   - Bit 3 (0x08): Exif metadata flag
   - Bit 4 (0x10): XMP metadata flag
6. **Chunk allowlist** — the only permitted FourCCs for a still image are: `VP8X`, `ALPH`, `VP8 ` (with trailing space), `VP8L`. Any other FourCC → reject. This explicitly rejects: `EXIF`, `ICCP`, `XMP `, `ANIM`, `ANMF`, and all unknown chunks.
7. Exactly one of `VP8 ` or `VP8L` must be present. Zero or more than one → reject.
8. `ALPH` may appear only if `VP8X` is present. `ALPH` without `VP8X` → reject.
...
**Primary control:**
- Spike function reads dimensions via §7 header parser before any transformation call.
...
**Secondary evidence (informational):** What does imgproxy return if F-REJECT is submitted directly? Record HTTP status. Supabase limits (25 MB, 50 MP) do not cover Forkensics' limit; imgproxy will not independently reject F-REJECT.
...
**Assertions:**
- HTTP 200 (no 546)
- No full raster decoded in function; no base64 encoding
...
|-------|---------|
| < 100 MB | **PASS** |
| 100 MB – < 200 MB | **INCONCLUSIVE** — acceptable at platform limit but requires a separate architecture decision before Rev 19; spike cannot be declared passed automatically |
...
**Implementation requirement:** Raw `fetch()` with streaming body reader and byte counter. The Supabase SDK `download()` returns a fully-buffered `Blob` before returning; it cannot enforce a limit pre-buffer and must not be used.

**Proposed maximum transformed-output size:** 5 MB (configurable; confirmed or adjusted by P-8).

**Assertions:**
- Response body exceeding the byte limit is aborted before full buffer completes
...
**Method:** Buffer transformed bytes via bounded streaming reader → compute SHA-256 → upload → download from `display_storage_path` → compute SHA-256 over downloaded bytes → compare.
...
3. **Confirm deletion via two independent methods:**
   ...
   **Method A — Authenticated GET:**  
   ...
   | 200 | Retry: wait 1 s, re-issue GET; repeat up to **3 total attempts** (initial + 2 retries). If 200 persists after all attempts → **FAIL** |
   | 401 / 403 / 429 / 5xx | Immediate **FAIL**; do not retry |

   **Method B — Service-role list query (independent confirmation):**  
   Issue a service-role-authorized Storage list for the exact key (not the prefix). Confirm the key is absent from the result. 404 or an empty listing = confirmed absent. Any error from the list call → record as inconclusive (does not override Method A result).

4. After both Method A confirms 404 and Method B confirms key absent: log `original_delete_confirmed`.
5. Log `would_advance_session`.

**Event sequence required:** `display_written` → `original_delete_confirmed` → `would_advance_session`

**Assertions:**
- Method A returns exactly 404 within the allowed attempts
- Method B confirms key absent
...
**Method:** Inject a mock JPEG response at the byte-inspection step. Confirm the function checks bytes 0–3 and 8–11 before writing.
...
| P-1 | Signed URL delivers WebP; unsigned URL and anon credentials rejected; anon + auth-user spike function calls return 403 | YES | ⬜ |
| P-2 | RIFF/WEBP magic confirmed; not JPEG | YES | ⬜ |
| P-3 | Zero prohibited groups, zero comment tag names, no non-allowlisted chunks; preflight valid | YES | ⬜ |
| P-4 | Orientation baked in; output correctly oriented; no EXIF chunk | YES | ⬜ |
| P-5 | Output dimensions and aspect ratio correct | YES | ⬜ |
| P-6 | Truncated → non-200; animated → pre-transform reject; polyglot → all structural conditions | YES | ⬜ |
| P-7 | Oversized rejected before transform; `transform_fetch_attempted=false`; no URL logged | YES | ⬜ |
...
| P-11 | Method A returns exactly 404; Method B confirms absent; event sequence correct; no session mutations | YES | ⬜ |
| P-12 | Non-WebP rejected by byte inspection before storage write | YES | ⬜ |
| P-13 | Latency recorded | NO | ⬜ |
| P-14 | Cost model documented | NO | ⬜ |

**Spike passes if:** All hard-gate criteria satisfied with no INCONCLUSIVE verdicts. An INCONCLUSIVE P-8 result blocks the spike from passing and requires a separate architecture decision before Rev 19.
...
`tools/image-spike/gate2b-evidence-r18-<YYYYMMDDTHHMMSSZ>/`
...
Magic phrase: `APPROVED: Gate 2B Rev 18 §1.2 — Feasibility Spike Proposal`

Authorizes: P-0 plan check and artifact drafting (if P-0 passes without upgrade). Does not authorize deployment, transformation requests, storage mutations, or plan upgrade.
