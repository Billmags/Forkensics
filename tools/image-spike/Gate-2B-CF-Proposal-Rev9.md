...
## Proposal Rev 9 — 2026-08-15

**Supersedes:** Rev 8 (not approved — 3 blockers, Codex SHA-256 `b8f4d0ff8e2df54b3b5a116d8d564def23ed75ed0e273d72d99da4e3df120e7b`)
...
| `package.json` | ⬜ pending | ⬜ pending |
| `package-lock.json` | ⬜ pending | ⬜ pending |
| `tsconfig.json` | ⬜ pending | ⬜ pending |
| `vitest.config.ts` | ⬜ pending | ⬜ pending |
| `worker-configuration.d.ts` | ⬜ pending | ⬜ pending |

**Pinned tool versions** (recorded at lock time — any change requires re-approval):

| Tool | Pinned version | Record command |
|------|---------------|----------------|
| Node | ⬜ pending | `node --version` |
| npm | ⬜ pending | `npm --version` |
| Wrangler | ⬜ pending | `./node_modules/.bin/wrangler --version` |
| TypeScript | ⬜ pending | `./node_modules/.bin/tsc --version` |
| Vitest | ⬜ pending | from `package.json` `devDependencies` |
| Workers Vitest (`@cloudflare/vitest-pool-workers`) | ⬜ pending | from `package.json` `devDependencies` |
...
4. `gitleaks git --no-banner` exits 0, zero findings. Required because the runner handles two credentials (`CLOUDFLARE_API_TOKEN` and `SPIKE_SECRET`).
...
```typescript
// src/index.ts — forkensics-image-spike Rev 9 — SPIKE ONLY
...
*Proposal Rev 9 — 2026-08-15 — awaiting Codex and Bill Phase 1 sign-off*
