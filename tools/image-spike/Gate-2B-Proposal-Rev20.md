# Gate 2B Proposal — Rev 20
# Managed Supabase Image Transformation Feasibility Spike

**Date:** 2026-08-15  
**Status:** DRAFT — awaiting Codex review and three-party approval  
**Predecessor:** Gate-2B-Proposal-Rev19.md (NOT APPROVED — two blockers)  
**Authorized project:** `hkfrbdpedrxmbsawnbpr` (forkensics-dev only)

**Rev 20 blockers closed from Rev 19:**

| # | Blocker | Section changed |
|---|---------|----------------|
| B-1 | `VP8X, VP8L` added as valid extended-lossless sequence; `VP8L` described as lossless with alpha potentially embedded in bitstream; `ALPH` forbidden with `VP8L`; lossy alpha consistency rule added: `ALPH` present iff VP8X alpha flag `0x10` is set | P-6 |
| B-2 | Hosted `role=authenticated` test removed; `role=authenticated` case covered by handler/unit test (same `role !== service_role` branch as anon); Auth user creation removed from §3.3 and cleanup; hosted auth matrix is three cases: no credentials, anon JWT, service-role JWT | §3.3, §3.6, §3.7, P-1 |
...
This is a **feasibility spike**, not an implementation. Each requirement must be proved independently before a Rev 21 implementation proposal is drafted.
...
- Upload of test fixtures to `game-media` under `gate2b-r20/<run-id>/`
- Execution of the spike protocol (P-1 through P-14)
- Deletion of all spike objects and the deployed function per §3.6

No temporary Auth users are created on the hosted project. The `role=authenticated` rejection case is verified by handler unit test in Phase 2 artifact review (see §3.7).
...
- Creation of Auth users, `public.profiles` rows, or `private.profile_suspensions` rows in `hkfrbdpedrxmbsawnbpr`

### 3.5 Authorized mutations and storage policy

- **Bucket:** `game-media` (existing private bucket)
- **Object prefix:** `gate2b-r20/<run-id>/` where `<run-id>` is a UUID generated at spike start
...
2. **List** all objects under `gate2b-r20/<run-id>/` in `game-media`, paginating all result pages.
...
6. Preserve evidence directory (`gate2b-evidence-r20-<YYYYMMDDTHHMMSSZ>/`) before exit.
...
**Hosted caller auth matrix (three cases, tested in P-1):**
...
**`role=authenticated` case:** Anon and authenticated-user JWTs execute the same `role !== service_role` handler branch. The authenticated-user rejection is verified by handler unit test in Phase 2 artifact review; a hosted test requiring Auth user creation is not authorized for this spike (see §3.3 and §3.4 regarding profile and suspension table side-effects).
...
**Objective:** Confirm private-bucket transformation succeeds with service-role authorization, and that all three hosted auth cases behave correctly.
...
7. Send a valid request to the spike function with the **service-role JWT**; confirm successful processing (2xx).
...
**`role=authenticated` case:** Covered by handler unit test in Phase 2 artifact review. Anon and authenticated-user JWTs reach the same handler branch (`role !== service_role`); a hosted test is not authorized for this spike due to `public.profiles` and `private.profile_suspensions` side-effects from Auth user creation.

**Pass:** All six hosted assertions satisfied.  
...
7. **Structural ordering and consistency rules:**
   - At most one `VP8X` chunk; at most one `ALPH` chunk. Duplicates → reject.
   - If `VP8X` is present, it must be the **first** chunk. `VP8X` appearing after any other chunk → reject.
   - `ALPH` is permitted only with `VP8 ` (lossy), never with `VP8L`. `ALPH` before `VP8L` → reject.
   - `ALPH` without `VP8X` → reject.
   - **Lossy alpha consistency:** If `ALPH` is present, the VP8X alpha flag (`0x10`) must be set; if `ALPH` is absent and the image chunk is `VP8 `, the VP8X alpha flag must be `0`. Mismatch → reject.
   ...
   **Valid complete chunk sequences for a sanitized still WebP:**
   ...
   | 2 | `VP8L` | Simple lossless; alpha may be embedded in the VP8L bitstream |
   | 3 | `VP8X`, `VP8 ` | Extended lossy, no alpha; VP8X alpha flag (`0x10`) must be `0` |
   | 4 | `VP8X`, `ALPH`, `VP8 ` | Extended lossy with alpha; VP8X alpha flag (`0x10`) must be `1` |
   | 5 | `VP8X`, `VP8L` | Extended lossless; alpha may be embedded in VP8L bitstream; no `ALPH` chunk permitted |

   Any sequence not in this table → reject.
...
|-------|---------|
| < 100 MB | **PASS** |
| 100 MB – < 200 MB | **INCONCLUSIVE** — requires separate architecture decision before Rev 21; spike cannot be declared passed automatically |
...
3. **Confirm deletion via two independent methods:**
   ...
   **Method A — Authenticated GET:**  
   ...
   | Any other status | Immediate **FAIL**; do not retry |

   **Method B — Service-role list query (independent confirmation):**  
   Issue a service-role-authorized Storage list for the exact object key. A conclusive empty or not-found result is required; any error or inconclusive result from Method B prevents P-11 from passing.
...
| P-1 | Six hosted assertions correct; unsigned URL rejects anon credentials; signed URL delivers WebP; `role=authenticated` via unit test | YES | ⬜ |
...
| P-6 | Truncated → non-200; animated → pre-transform reject; polyglot → all structural conditions; five valid sequences; VP8X flags and consistency rules | YES | ⬜ |
...
**Spike passes if:** All hard-gate criteria satisfied with no INCONCLUSIVE verdicts. An INCONCLUSIVE P-8 result blocks the spike from passing and requires a separate architecture decision before Rev 21.
...
`tools/image-spike/gate2b-evidence-r20-<YYYYMMDDTHHMMSSZ>/`
...
Magic phrase: `APPROVED: Gate 2B Rev 20 §1.2 — Feasibility Spike Proposal`
...
| Party | Status |
|-------|--------|
| Claude | ✅ APPROVED 2026-08-15 — VP8X/VP8L sequences complete; flag masks correct; RIFF bounds correct; all fifteen P-steps reviewed; hosted auth-user exclusion rationale sound |
| Codex | ✅ APPROVED 2026-08-15 — "APPROVED: Gate 2B Rev 20 §1.2 — Feasibility Spike Proposal"; reviewed SHA-256 `2d422458391a3d7ff194179e99681bb07af447a0159fcad5ff4ff7b80345756d`; attachment and repository copy byte-identical |
| Bill | ✅ APPROVED 2026-08-15 — "APPROVED - Bill" |
