...
**Self-contained:** This document is the complete approved contract. No prior revision (Rev 1–11) need be consulted for implementation.
**Codex Rev 10 hash:** `d5117dc69273a231c844fe0911eda4e8d06870e066cee8d328cae8741e155f10`

**Changes from Rev 11:**
1. `transfer_group_ownership` contract corrected: V1 has no `groups.owner_id`; ownership is `group_members.role = 'owner'` (enforced by the `one_owner_per_group` partial unique index). Full 7-step atomic sequence added: group lock → identify current owner → lock recipient membership → validate recipient → INSERT audit row → UPDATE old owner to 'member' → UPDATE new owner to 'owner'.
2. T11.8a added: positive transfer test verifying both role changes and the audit row.
3. Success criteria updated to match corrected ownership mechanism.
...
```
1. Lock the group row: SELECT id FROM public.groups WHERE id = p_group_id FOR UPDATE;
2. Identify the current owner:
     SELECT id FROM public.group_members
     WHERE group_id = p_group_id AND role = 'owner' FOR UPDATE;
   Verify that current_owner = private.auth_uid() (or the verified caller).
   If NOT FOUND or mismatch → FK_UNAUTHORIZED.
3. Lock the recipient's membership row:
     SELECT id, player_id FROM public.group_members
     WHERE group_id = p_group_id AND player_id = p_new_owner_id FOR UPDATE;
...
5. INSERT INTO public.group_ownership_history
   ...
   VALUES
     (p_group_id, current_owner_id, p_new_owner_id);
6. UPDATE public.group_members SET role = 'member'
   WHERE group_id = p_group_id AND player_id = current_owner_id;
