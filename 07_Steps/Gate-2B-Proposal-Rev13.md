# Gate 2B Proposal — Rev 13 — Pixel Ceiling Discovery Spike on forkensics-dev

**Status:** DRAFT — awaiting three-party approval

**Governance gate:** Three-party approval (Bill + Claude + Codex) required before any cloud operation is executed. The magic words are `APPROVED: Gate 2B Rev 13 — Pixel Ceiling Discovery Spike`.

**Authorized by:** Step 27 Rev 5 §3 Gate 2 Phase B.

**Supersedes:** Gate 2B Rev 12 (DRAFT — 5 executable blockers identified by Codex; never approved).

**Rev 13 changes from Rev 12 (all 5 Codex blockers addressed + recommended hardening):**

1. **Blocker 1 — ICC crash fixed.** `ImageCms.createProfile("sRGB")` returns a `CmsProfile` object with no `tobytes()` method. Corrected to `ImageCms.ImageCmsProfile(ImageCms.createProfile("sRGB")).tobytes()`, which wraps the profile in the `ImageCmsProfile` class that exposes `tobytes()`.

2. **Blocker 2 — Confirmation JPEG quality loop no longer oscillates.** The bidirectional `q -= 3` / `q += 3` approach can oscillate forever when adjacent quality values straddle the band. Replaced with a finite downward sweep (qualities 95 → 50, step −1) that collects all candidates meeting the band and selects the highest-quality match. If no match is found, the loop terminates and exits with a fatal error.

3. **Blocker 3 — Survey telemetry no longer poisons the final verdict.** `validate_telemetry()` has been split into two distinct functions:
   - `survey_telemetry_result(label)` — returns one of three outcomes without ever calling `fail()`:
     - `"evidence_fail"` if telemetry is inconclusive, malformed, or contains an unrecognized `shutdown_reason` → calls `fail()` (evidence quality problem, not just threshold)
     - `"nonviable"` if CPU > 1,500 ms, MiB > 200, or `shutdown_reason` is a resource-limit value → records the level as nonviable without touching `GATE2B_PASS`
     - `"pass"` if all thresholds met and reason is an accepted value
   - `confirmation_telemetry_result(label)` — any failure calls `fail()`, as confirmation failures are hard gate failures.

4. **Blocker 4 — Ceiling selector requires reason in OK set.** The Python selector previously used `reason not in RESOURCE_REASONS`, which admitted unrecognized values. Changed to positive allowlist: a level is viable only if `reason in {"EventLoopCompleted", "EarlyDrop", "TerminationRequested"}`. This aligns with the validated telemetry boolean passed from bash.

5. **Blocker 5 — C-REJECT 546 treated as confirmation failure.** The C-4 caller now captures `invoke_case`'s return code for C-REJECT. Exit code 2 (546) calls `fail()` and exits rather than reaching a PASS verdict.

**Recommended hardening (also applied):**
- WebP GPS verification now parses the EXIF chunk data with `piexif` to confirm a GPS IFD is present, not just that an `EXIF` chunk exists.
- RIFF parser validates declared chunk size against remaining data; a chunk claiming more bytes than available raises `ValueError` rather than silently slicing.
- C-REJECT byte ceiling enforced: preflight confirms `os.path.getsize(reject_path) <= 500_000`.
...
- `tools/image-spike/gate2b-run-r13.sh`
- `tools/image-spike/gate2b-fixtures-r13.py`
...
- `bash -n tools/image-spike/gate2b-run-r13.sh` — exit 0
- `python3 -m py_compile` for all three Python scripts — exit 0
- `gitleaks detect` — no findings

### 1.3 Local Edge Runtime Verification

Run `bash tools/image-spike/gate2b-local-test-r13.sh` before Phase S-1. S-5 (`image_decode_started: true`) and a rejection fixture (`image_decode_started: false`) must pass.

---

## Section 2 — Purpose

Gate 2B Rev 9 proved 20 MP exceeds Supabase's 256 MiB memory ceiling (293.5 MiB peak). Rev 13 finds the highest pixel count satisfying both:
...
A 546 at survey level N stops ascent without failing the gate; ceiling selection proceeds with data from levels below N. A 546 with no viable lower level is a hard Gate FAIL. Confirmation phase 546s are hard Gate FAILs.
...
### 4.2 Handler Pipeline (unchanged from Rev 12)
...
Confirmation JPEG byte bands match the survey fixture for the chosen MP level.
...
C-REJECT: `(ceiling_w + 1) × ceiling_h` pixels. Solid color, quality=1. Byte ceiling: ≤ 500,000.
...
| S16 | `cpu_time_used` (ShutdownEvent) | ≤ 1,500 ms (nonviable, no gate fail) |
| S17 | `memory_used.total` (ShutdownEvent ÷ 1,048,576) | ≤ 200 MiB (nonviable, no gate fail) |
| S18 | `shutdown_reason` (ShutdownEvent) | in OK set (nonviable if resource-limit; gate fail if unknown) |

### 6.2 Ceiling Selection Criterion

A survey level is **viable** if: S1–S15 all passed (functional) AND `survey_telemetry_result()` returned `"pass"`. The **selected ceiling** is the highest viable level.
...
| C2 | C-JPEG-1,2,3 and C-WEBP: `cpu_time_used` | ≤ 1,500 ms (hard fail) |
| C3 | C-JPEG-1,2,3 and C-WEBP: `memory_used.total` | ≤ 200 MiB (hard fail) |
| C4 | C-JPEG-1,2,3 and C-WEBP: `shutdown_reason` | in OK set (hard fail) |
| C5 | C-JPEG-1,2,3 byte size | within survey band for chosen MP |
| C6 | C-WEBP cold wall time | ≤ 30 s |
| C7 | C-REJECT: HTTP status | 200 (546 → hard Gate FAIL) |
| C8 | C-REJECT: `accepted` | false |
| C9 | C-REJECT: `reason` | `pre_decode_rejected` |
| C10 | C-REJECT: `image_decode_started` | false |
| C11 | C-REJECT: `pixel_count` | `(ceiling_w + 1) × ceiling_h` |
...
- HTTP 546 in any confirmation phase (C-1 through C-4, including C-REJECT)
...
- Survey telemetry: `shutdown_reason` unrecognized (evidence quality failure)
- Survey telemetry: inconclusive or malformed fields

**Survey 546 boundary (not a hard stop if lower viable level exists):** 546 at level N breaks ascent. Ceiling selection proceeds with levels below N.

**Survey telemetry nonviable (no gate poison):** CPU > 1,500 ms, MiB > 200, or `shutdown_reason` is a resource-limit value → level recorded as nonviable; `GATE2B_PASS` unchanged.

**Soft failures (accumulated; verdict is FAIL):** Any non-546 non-auth HTTP error; any S1–S14 assertion miss; wall-time exceeded.

**INCONCLUSIVE (evidence quality failure → hard fail):** execution_id not a valid UUID; ShutdownEvent not found; any telemetry field absent or non-numeric; operator enters `INCONCLUSIVE`.
...
Delete the function first (logs persist). Then perform two-step lookup.

### 8.2 Two-Step Log Lookup

**Step 1:** Dashboard → edge-logs → filter `metadata.function_id = 'image-spike'` → search `run_id` → find LogEvent → record `execution_id`.
...
| `Memory` | Resource limit | nonviable (no gate fail) | hard Gate FAIL |
| `CPUTime` | Resource limit | nonviable (no gate fail) | hard Gate FAIL |
| `WallClockTime` | Resource limit | nonviable (no gate fail) | hard Gate FAIL |
| Any other | Unknown | hard Gate FAIL | hard Gate FAIL |
...
# Per level receives: func_pass ("true"/"false"/""), cpu (ms float str),
# mem (bytes float str), reason (str), telem_ok ("true"/"false")
# telem_ok is set by the runner: "true" only if survey_telemetry_result == "pass"

viable = []
for mp in MP:
    fp      = survey_func_pass[mp]   # "true" / "false" / ""
    telem   = survey_telem_ok[mp]    # "true" / "false"
    cpu_r   = survey_cpu[mp]
    mem_r   = survey_mem[mp]
    reason  = survey_reason[mp]

    if fp != "true" or telem != "true":
        continue
    try:
...
Unchanged from Rev 9 (verifies that output WebP is metadata-clean).

### 10.2 gate2b-fixtures-r13.py

```python
#!/usr/bin/env python3
"""gate2b-fixtures-r13.py — Rev 13 fixture generator.
Usage:
  python3 gate2b-fixtures-r13.py <out_dir>
  python3 gate2b-fixtures-r13.py <out_dir> confirm <cw> <ch> <cp> <min_b> <max_b>
...
def _exif_bytes() -> bytes:
    exif = {
        "0th": {
            ...
            piexif.ImageIFD.Model:    b"Rev13",
            piexif.ImageIFD.Software: b"gate2b-fixtures-r13",
        },
        "Exif": {
...
def _iptc_app13() -> bytes:
    ...
    caption = b"Gate2B-Rev13-IPTC-Test"
...
def _inject_comment(jpeg: bytes, text: bytes = b"Gate2B-Rev13-COMMENT") -> bytes:
    hdr = b"\xff\xfe" + struct.pack(">H", 2 + len(text)) + text
    return jpeg[:2] + hdr + jpeg[2:]

# ---------------------------------------------------------------------------
# RIFF chunk parser — validates declared size against remaining data
# ---------------------------------------------------------------------------
def _parse_riff_chunks(data: bytes) -> dict:
    ...
    if file_size + 8 > len(data):
        raise ValueError(
            f"RIFF declared size {file_size} exceeds actual data length {len(data)-8}")
    pos = 12
    out: dict = {}
    while pos + 8 <= len(data):
        ...
        if pos + 8 + size > len(data):
            raise ValueError(
                f"Chunk {tag!r} at offset {pos} claims {size} bytes but only "
...
def _write_webp_full(img: Image.Image, quality: int, path: str) -> int:
    ...
    raw = buf.getvalue()
    chunks = _parse_riff_chunks(raw)   # raises ValueError on boundary violation
...
def generate_survey(out_dir: str) -> None:
    ...
    print("Gate 2B Rev 13 survey fixture generation ...")
...
def generate_confirmation(out_dir: str, cw: int, ch: int, cp: int,
    ...
    for i, seed in enumerate([42, 43, 44], start=1):
        ...
        candidates = []   # (quality, size) pairs meeting the band
        ...
        if not candidates:
            print(f"FATAL: C-JPEG-{i} (seed={seed}) cannot meet band "
                  ...
                  file=sys.stderr)
            sys.exit(1)
        # Pick highest quality (first candidate since we swept high to low)
        best_q, best_size = candidates[0]
        _write_jpeg_full(img, best_q, path)   # write the winner
        sha = hashlib.sha256(open(path, "rb").read()).hexdigest()
...
### 10.3 gate2b-verify-input-metadata.py (Rev 13)

Key change from Rev 12: `_webp_has_gps()` now parses the EXIF chunk bytes with `piexif` to confirm a GPS IFD is present. RIFF parser validates chunk boundaries.

```python
#!/usr/bin/env python3
"""gate2b-verify-input-metadata.py — structural metadata verification (Rev 13).
Usage:
...
# ---------------------------------------------------------------------------
# RIFF/WebP chunk parser — validates declared chunk boundaries
# ---------------------------------------------------------------------------
def _parse_riff_chunks(data: bytes) -> dict:
    if len(data) < 12 or data[:4] != b"RIFF" or data[8:12] != b"WEBP":
        raise ValueError("Not a RIFF/WEBP container")
    ...
    if file_size + 8 > len(data):
        raise ValueError(
            f"RIFF declared size {file_size} exceeds actual length {len(data)-8}")
    pos = 12
    out: dict = {}
    while pos + 8 <= len(data):
        tag  = data[pos:pos+4]
        ...
        if pos + 8 + size > len(data):
            raise ValueError(
                f"Chunk {tag!r} at {pos} claims {size}B but only "
                f"{len(data)-pos-8} remain")
        out.setdefault(tag, []).append(data[pos+8:pos+8+size])
        pos += 8 + size + (size & 1)
    return out
...
def _webp_has_gps(chunks: dict) -> bool:
    ...
    for exif_data in chunks.get(b"EXIF", []):
        # Pillow's WebP EXIF chunk omits the "Exif\x00\x00" prefix present in JPEG APP1
        # piexif.load accepts either raw TIFF bytes or the JPEG-style b"Exif\x00\x00" prefix
...
## Section 11 — Runner: gate2b-run-r13.sh

```bash
#!/usr/bin/env bash
# tools/image-spike/gate2b-run-r13.sh — Gate 2B Rev 13 test runner
# DISPOSABLE — Gate 2B only. Execute only after three-party approval of Rev 13.
#
# Usage: PROJECT_REF=hkfrbdpedrxmbsawnbpr ANON_KEY=yyy bash tools/image-spike/gate2b-run-r13.sh
...
IMG_DIR="$REPO_ROOT/tools/image-spike/test-images-r13"
FIXTURES_PY="$SCRIPT_DIR/gate2b-fixtures-r13.py"
VERIFY_OUT_PY="$SCRIPT_DIR/gate2b-verify-metadata.py"
VERIFY_IN_PY="$SCRIPT_DIR/gate2b-verify-input-metadata.py"

TIMESTAMP="$(date -u +%Y%m%dT%H%M%SZ)"
RESULTS_DIR="$REPO_ROOT/tools/image-spike/gate2b-evidence-r13-${TIMESTAMP}"
...
declare -A SURVEY_FUNC_PASS=()    # "true" / "false" per MP
declare -A SURVEY_TELEM_OK=()     # "true" / "false" — set by survey_telemetry_result
...
cleanup() {
  ...
  echo "" >&2; echo "=== Gate 2B Rev 13 Cleanup (trigger: $trigger) ===" >&2
...
if not (math.isfinite(mb) and mb>=0): print(f"FAIL: {sys.argv[1]} not finite"); sys.exit(1)
...
# ---------------------------------------------------------------------------
# invoke_case
# Returns: 0 = completed (failures recorded via fail()); 2 = HTTP 546 (no fail() here)
# 546 handling is ALWAYS the caller's responsibility.
# ---------------------------------------------------------------------------
invoke_case() {
  LAST_RUN_ID=""
  LAST_INVOKE_546=false
...
# ---------------------------------------------------------------------------
# Telemetry: survey variant — never calls fail() for threshold violations
# Returns: "pass" | "nonviable" | "evidence_fail" (via stdout)
# Only "evidence_fail" causes fail() to be called.
# ---------------------------------------------------------------------------
OK_REASONS="EventLoopCompleted EarlyDrop TerminationRequested"
...
survey_telemetry_result() {
  ...
  results_append "execution_id: $TELEM_EXEC_ID  cpu: $TELEM_CPU  mem: $TELEM_MEM  reason: $TELEM_REASON"

  # Evidence quality check — any INCONCLUSIVE or malformed is evidence_fail (calls fail())
  ...
  # Resource-limit reason → nonviable (not a gate failure)
  if echo "$RESOURCE_REASONS" | grep -qw "${TELEM_REASON:-}"; then
    results_append "$label: nonviable (resource-limit reason: $TELEM_REASON)"
    echo "nonviable"; return; fi

  # Numeric threshold check → nonviable (not a gate failure)
  local outcome
  outcome=$(python3 - "$TELEM_CPU" "$TELEM_MEM" "$MEM_THRESHOLD_MIB" "$CPU_THRESHOLD_MS" <<'PYEOF'
import sys, math
cpu_r,mem_r,mt,ct = sys.argv[1:]
mt,ct = float(mt),float(ct)
try: cpu=float(cpu_r); mem=float(mem_r)
except ValueError as e: print(f"evidence_fail:{e}"); sys.exit(0)
if not (math.isfinite(cpu) and cpu>=0) or not (math.isfinite(mem) and mem>=0):
    print("evidence_fail:non-finite"); sys.exit(0)
mib=mem/1_048_576
if cpu>ct or mib>mt:
    print(f"nonviable:cpu={cpu:.0f}ms mem={mib:.1f}MiB")
else:
    print(f"pass:cpu={cpu:.0f}ms mem={mib:.1f}MiB reason=ok")
PYEOF
  )

  case "${outcome%%:*}" in
    evidence_fail)
      fail "$label: survey telemetry malformed — ${outcome#*:}"
      results_append "$label: evidence_fail"
      echo "evidence_fail" ;;
    nonviable)
      results_append "$label: nonviable — ${outcome#*:}"
      echo "nonviable" ;;
    pass)
      results_append "$label: pass — ${outcome#*:}"
      echo "pass" ;;
    *)
      fail "$label: survey_telemetry_result internal error (output='$outcome')"
      echo "evidence_fail" ;;
  esac
}
...
if not (math.isfinite(cpu) and cpu>=0) or not (math.isfinite(mem) and mem>=0):
    print(f"FAIL: non-finite values"); sys.exit(1)
mib=mem/1_048_576
...
PYEOF
  local te=$?
  if [[ $te -ne 0 ]]; then
    fail "$label: confirmation telemetry threshold check failed"; return 1; fi
...
results_append "# Gate 2B Results — Rev 13"
...
for i, mp in enumerate(MP):
    # (func_pass, telem_ok, cpu, mem, reason) — 5 values per level
...
for mp in MP:
    ...
    try:
        ...
        print(f"{mp:>4}  {fp:>5}  {tok:>5}  {'INC':>8}  {'INC':>8}  {reason:>22}  {'NO':>7}")
        continue
    mib = mem / MIB
    # Require reason in OK set (not just absent from RESOURCE set)
...
echo "Ceiling: ${CHOSEN_MP} MP (${CHOSEN_W}×${CHOSEN_H}, ${CHOSEN_PX} px)"
...
read -rp "Type YES to confirm three-party approval and proceed: " _cc
...
# C-REJECT — capture return code; 546 = confirmation failure
...
if [[ $reject_exit -eq 2 ]]; then
  fail "C-REJECT: HTTP 546 — confirmation failure (C-REJECT should never touch the runtime ceiling)"
fi
...
confirmation_telemetry_result "C-WEBP" || fail "C-WEBP: confirmation telemetry gate failed"
...
1. `python3 gate2b-fixtures-r13.py test-images-r13/` — survey fixtures, verify sizes and metadata
2. `bash gate2b-local-test-r13.sh` — S-5 (`image_decode_started: true`), reject (`false`)
3. `deno fmt --check`, `deno lint`, `deno check` — zero findings
4. `bash -n tools/image-spike/gate2b-run-r13.sh` — exit 0
...
`CANONICAL_PIXEL_LIMIT` established by Rev 13 is the mandatory upload pixel ceiling.
...
All 5 Codex blockers from Rev 12 are addressed, plus recommended hardening applied. No cloud operation has been performed.

Requested action: three-party sign-off (Bill + Claude + Codex) using the magic words:

**`APPROVED: Gate 2B Rev 13 — Pixel Ceiling Discovery Spike`**
