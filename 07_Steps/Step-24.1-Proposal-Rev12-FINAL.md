...
**Self-contained:** This document is the complete approved contract. No prior revision (Rev 1–12) need be consulted for implementation.
**Codex Rev 10 hash:** `d5117dc69273a231c844fe0911eda4e8d06870e066cee8d328cae8741e155f10`

**Changes from Rev 12:**
1. `transfer_group_ownership` steps 2 and 3 corrected: `group_members` has no `id` column; primary key is composite `(group_id, player_id)`. Step 2 now uses `SELECT gm.player_id INTO v_current_owner_id FROM public.group_members gm WHERE gm.group_id = p_group_id AND gm.role = 'owner' FOR UPDATE`. Step 3 now uses `PERFORM 1 FROM public.group_members gm WHERE gm.group_id = p_group_id AND gm.player_id = p_new_owner_id FOR UPDATE`.
...
```
1. Lock the group row: SELECT group_id FROM public.groups WHERE id = p_group_id FOR UPDATE;
