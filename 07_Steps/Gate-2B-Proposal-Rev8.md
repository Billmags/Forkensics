# Gate 2B Proposal — Rev 8 — Hosted magick-wasm Spike on forkensics-dev

**Status:** DRAFT — awaiting three-party approval

**Governance gate:** Three-party approval (Bill + Claude + Codex) required before any cloud operation is executed. The magic words are `APPROVED: Gate 2B Rev 8 — Hosted magick-wasm Spike`.

**Authorized by:** Step 27 Rev 5 §3 Gate 2 Phase B.

**Supersedes:** Gate 2B Rev 7 (rejected — 4 execution blockers).

**Rev 8 changes from Rev 7 (all 4 blockers addressed):**

1. **Correct project ref (Blocker 1):** `APPROVED_PROJECT_REF` was set to the project *name* `forkensics-dev`. The actual project ref is `torkgydbvktqebssfpdi`. Updated in: `APPROVED_PROJECT_REF`, usage comment, `FUNC_URL` hostname, all dashboard URLs.

2. **REMOTE_OWNED flag replaces DEPLOY_ATTEMPTED (Blocker 2):** A single `REMOTE_OWNED` boolean tracks whether this run currently owns a live deployment. It is set to `true` immediately after a successful absence check + deploy, and to `false` immediately after a confirmed deletion. The cleanup handler may delete only when `REMOTE_OWNED=true`. This prevents cleanup from touching a function that was deployed independently between Phase 0 deletion and Phase 1 deployment.

3. **Bundle parsing truly anchored (Blocker 3):** Replaced two-step grep with a single Python `re.search` that captures the number and unit immediately following the `script size:` label — `(?:script|bundle) size:\s*([0-9]+(?:\.[0-9]+)?)(MiB|MB)\b` — so no intervening token between label and value can be picked up.

4. **Correct ShutdownEvent memory field (Blocker 4):** `WorkerMemoryUsed` is a separate diagnostic event, not a field on ShutdownEvent. ShutdownEvent contains `memory_used.total` (bytes) plus `execution_id`. Operator now extracts `memory_used.total` in bytes from the ShutdownEvent; the runner converts to MiB (`÷ 1,048,576`) and applies the 220 MiB threshold. All labels, prompts, validation, and §8.2 procedure updated accordingly.
...
```bash
#!/usr/bin/env bash
# tools/image-spike/gate2b-local-test.sh — Rev 8 (unchanged from Rev 7)
set -uo pipefail
...
Two deployments of `image-spike` on project ref `torkgydbvktqebssfpdi` (forkensics-dev), in sequence:
...
1. Open: `https://supabase.com/dashboard/project/torkgydbvktqebssfpdi/logs/edge-logs`
...
```bash
#!/usr/bin/env bash
# tools/image-spike/gate2b-run.sh — Gate 2B test runner, Rev 8
# DISPOSABLE — Gate 2B only. Execute only after three-party approval of Rev 8.
#
# Usage: PROJECT_REF=torkgydbvktqebssfpdi ANON_KEY=yyy bash tools/image-spike/gate2b-run.sh
...
# ---------------------------------------------------------------------------
APPROVED_PROJECT_REF="torkgydbvktqebssfpdi"
...
# ---------------------------------------------------------------------------
CLEANUP_RAN=false
# REMOTE_OWNED: true iff this run currently has a live image-spike deployed.
# Set true after pre-deploy absence check passes and deploy succeeds.
# Set false after delete_confirmed() succeeds.
# Cleanup deletes only when REMOTE_OWNED=true.
REMOTE_OWNED=false
WASM_COPIED=false
...
# Deletes: remote function (only if REMOTE_OWNED=true), config patch,
...
cleanup() {
  ...
  # Remote deletion — only if this run owns the deployment.
  # REMOTE_OWNED is false after delete_confirmed() succeeds, so this block
  # does not re-run functions list when deletion is already confirmed.
...
mkdir -p "$OUT_DIR"
> "$RESULTS_MD"
results_append "# Gate 2B Results — Rev 8"
...
# ---------------------------------------------------------------------------
# deploy_function
#   1. Pre-deploy absence check (sets REMOTE_OWNED=true only after passing)
#   2. Deploy
...
deploy_function() {
  ...
  # REMOTE_OWNED is set only after this check passes and deploy succeeds,
  # ensuring cleanup cannot touch a function this run did not deploy.
  ...
  # Set REMOTE_OWNED=true now that the deploy succeeded.
  # Any subsequent failure will trigger cleanup, which guards on this flag.
  REMOTE_OWNED=true
...
# Sets REMOTE_OWNED=false on success (prevents cleanup from re-running).
# ---------------------------------------------------------------------------
delete_confirmed() {
  local phase="$1"
  ...
  REMOTE_OWNED=false   # this run no longer owns any deployment
  return 0
}
...
**`APPROVED: Gate 2B Rev 8 — Hosted magick-wasm Spike`**
