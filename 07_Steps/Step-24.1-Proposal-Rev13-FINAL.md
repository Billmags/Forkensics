...
**Self-contained:** This document is the complete approved contract. No prior revision (Rev 1–13) need be consulted for implementation.
**Codex Rev 10 hash:** `d5117dc69273a231c844fe0911eda4e8d06870e066cee8d328cae8741e155f10`

**Changes from Rev 13:**
1. `transfer_group_ownership` step 1 corrected: `public.groups` has no `group_id` column; its PK is `id`. Step 1 now uses `PERFORM 1 FROM public.groups WHERE id = p_group_id FOR UPDATE` (no column selected; lock only).
