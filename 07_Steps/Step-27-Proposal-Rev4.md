# Step 27 Proposal — Rev 4 — Edge Function Implementation Plan

**Status:** Pending review (Claude → Codex → Bill approval)

**Governance gate:** All three parties (Bill + Claude + Codex) must approve before any TypeScript code is written. The magic words are `APPROVED: Step 27 Rev 4 — Edge Function Implementation Plan`.

**Supersedes:** Step 27 Rev 3 (rejected — 7 blockers)

**Rev 4 changes from Rev 3:**
1. Image safety limit made implementation-ready: decoded-memory budget calculated; concrete pre-decode pixel limits required from Gate 2; tests must vary pixel dimensions not just compressed file size; WASM packaging method (`static_files` in `config.toml`, Docker/CLI deployment, CLI ≥ 2.7.0) made mandatory.
2. JWT verification updated to `@supabase/server` pattern: `createSupabaseContext(req, { auth: 'user' })` for user-facing functions; raw JWT decoding prohibited; profile checks occur after verified identity; deletion carveout uses same pattern with different profile check; cron functions use `withSupabase({ auth: 'none' })` + manual cron-secret verification.
3. S3 local preflight corrected: `S3_REGION=local`, `S3_ACCESS_KEY_ID=stub`, `S3_SECRET_ACCESS_KEY=<ANON_KEY from supabase status -o env>`; preflight upload corrected to zero-byte object (calling it a zero-byte test, not one-byte).
4. `upload-complete` finalization transitions made explicit: three paths fully defined; re-resolve-after-finalization-error procedure added; test for "finalization commits but response is lost" added.
5. `scheduled-close` concurrent-worker race specified: `reveal_case_service_wrapper` is not idempotent on revealed cases; loser receives a known error (`state != 'locked'`); loser classifies as skipped; concurrent-worker test added proving exactly one scoring execution.
6. Swift binary-response example corrected to use the `decode: { data, _ in data }` closure form.
7. All Section 9.2 RPC placeholder entries replaced with exact V4 parameter names.
...
**Phase B — Hosted runtime validation (separate explicit cloud approval required):**

If Phase A passes, a disposable hosted spike on forkensics-dev measures actual hosted-runtime memory and CPU for the established canonical limits. Phase B requires its own three-party approval. It is NOT authorized under this step.
...
Gate 5 (Deno installation smoke)                           │
         │                                                 │
         ▼                                                 │
Step A: upload-authorize → Gate 6 (lockfile)               │
...
11. Call `finalize_upload_session(session_id, sha256_hash)`.
   ...
   **Finalization error handling:** If `finalize_upload_session` raises, re-resolve the session:
   - Resolved `complete` → the finalization committed despite the error; return `200` idempotent.
   - Resolved `sanitized` → finalization definitely did not commit; return `500 FK_INTERNAL`; client retries via sanitized branch on next call.
   - Other state or no row → fail-closed; return `500 FK_INTERNAL`.
...
16. Call `finalize_upload_session(session_id, sha256_hash)`. Apply same finalization error handling as steps 11a–11c above.
...
- **Finalization commits but response is lost:** Call again; re-resolve → `complete`; return idempotent 200.
- `FK_INVALID_HASH` from DB → 422.
- Finalization error, re-resolve `complete` → idempotent 200.
- Finalization error, re-resolve `sanitized` → 500; client retries via sanitized branch.
...
| `magick-wasm` at 10 MB | Gate 2: Phase A (local) → Phase B (hosted, separate cloud approval). Pixel budget established by spike. |
...
| Finalization-commits-response-lost | Re-resolve on finalization error; `complete` → idempotent 200. Test added. |
...
To approve: reply with `APPROVED: Step 27 Rev 4 — Edge Function Implementation Plan`.
