# Step 25 Proposal — Rev 3 — V2 Migration: Upload Session Infrastructure
...
**Changes from Rev 2 (addressing 9 Codex blockers):**
...
AS $$
BEGIN
  -- Require a non-NULL, strictly future expiry timestamp.
  -- A NULL or past expiry indicates a signing error; reject before writing any state.
  IF p_actual_storage_upload_expires_at IS NULL
     OR p_actual_storage_upload_expires_at <= now()
  THEN
...
- `p_actual_storage_upload_expires_at = now() - interval '1 second'` (past) → raises `FK_INVALID_INPUT`
- `p_actual_storage_upload_expires_at = now() + interval '1 hour'` (future), `status = 'pending'`, `storage_upload_expires_at = NULL` → sets expiry; returns void
...
- Session not found → no rows updated → raises `FK_WRONG_STATE`
...
**Race Test RT-1: reservation vs. deletion**
```
Session A:  BEGIN;
Session A:  SELECT reserve_upload_session(...);  -- holds challenge FOR UPDATE
Session B:  BEGIN;
Session B:  SELECT prepare_account_deletion_wrapper(p_uploader_id);
            -- must BLOCK here (session A holds challenge lock)
Session A:  ROLLBACK;   -- releases lock
Session B:  -- now unblocks; completes or raises FK_FORBIDDEN
Session B:  ROLLBACK;
```
Expected: Session B blocks until Session A releases; no deadlock; no corrupted row.
...
```
Session A:  BEGIN;
Session A:  SELECT reserve_upload_session(p_challenge_id := X, ...);  -- succeeds
Session B:  BEGIN;
Session B:  SELECT reserve_upload_session(p_challenge_id := X, ...);  -- FK_UPLOAD_IN_PROGRESS
Session A:  ROLLBACK;
Session B:  ROLLBACK;
```
Expected: Exactly one reservation succeeds; the other gets FK_UPLOAD_IN_PROGRESS.
