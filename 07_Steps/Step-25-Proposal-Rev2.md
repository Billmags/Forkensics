# Step 25 Proposal — Rev 2 — V2 Migration: Upload Session Infrastructure
...
**Changes from Rev 1:**
1. `media_objects` status constraint (Section 3) now includes `'pending_review'`, `'rejected'`, and `'removed'` — required by Step 24.1 moderation contracts.
2. `private.media_storage_keys.sha256_hash` column added (Section 3.5) using the two-migration plan from Step 24.1 Part 3.11.
3. `finalize_upload_session` (Section 5.7) updated: adds `p_sha256_hash text` parameter; validates format (`'^[0-9a-f]{64}$'`); sets new media_object `status = 'pending_review'` (not `'ready'`); stores `sha256_hash` in `private.media_storage_keys`.
4. Section 4.2 note updated to include `'pending_review'`, `'rejected'`, `'removed'` among rejected statuses.
5. Section 11 grant list updated with corrected `finalize_upload_session` signature.
6. Section 13.9 acceptance tests updated: `status = 'pending_review'`; SHA-256 validation and storage tests added.
...
- After finalize: `activate_challenge` trigger raises (status = 'pending_review', not 'ready')
- After `approve_photo` sets status → 'ready': `activate_challenge` trigger passes
...
- Session transitions to `complete` → `activate_challenge` upload-session trigger no longer blocks
- Session transitions to `failed` → `activate_challenge` upload-session trigger no longer blocks
- `media_object_id` pointing to `status = 'pending_review'` → media-ready trigger raises (moderator must approve first)
...
- [ ] `private.media_storage_keys.sha256_hash` two-migration plan agreed (V2a nullable; V2b NOT NULL after backfill)
- [ ] Both V2 triggers on `public.challenges` agreed including ordering; trigger correctly rejects `'pending_review'` and `'rejected'` media
...
- [ ] `finalize_upload_session` sets `status = 'pending_review'`; validates and stores `sha256_hash`; supersedes prior media regardless of prior status; `p_sha256_hash` parameter included
