# Step 25 Proposal — Rev 4 — V2 Migration: Upload Session Infrastructure
...
**Changes from Rev 3 (addressing 6 Codex blockers):**
...
3. `activate_upload_session` fully hardened: (a) fast-path rejects NULL/past timestamp before acquiring any lock; (b) enforces approved 5-minute signing window (`clock_timestamp() + interval '5 minutes' + interval '30 seconds'` tolerance); (c) locks session row `FOR UPDATE` before all state checks; (d) rejects if `expires_at <= clock_timestamp()` (session claim window expired); (e) rejects if capability expiry > session expiry. Section 13.4 acceptance tests updated to use a compliant 4-minute duration and cover the new blocker cases.
4. `fail_deletion_recovery` now validates `p_error_code` matches `^FK_[A-Z_]+$` and is ≤ 50 chars before calling `private.record_deletion_failure` — prevents arbitrary strings from being stored in the deletion failure log.
5. RT-1 split into two sub-tests: RT-1a (deletion wins challenge lock first → reservation fails with `FK_FORBIDDEN`) and RT-1b (reservation commits first → deletion quiesces the committed session). Both use automated harness with advisory-lock coordination, 10-second timeout guards, exit codes, and fixture cleanup.
6. RT-3 corrected: Session A now COMMITs before Session B attempts; unique-index conflict is real (not rolled back). Automated harness added matching RT-1 harness pattern.
...
AS $$
BEGIN
  -- Enforce FK_* format; no raw error strings or storage paths stored via this wrapper.
  IF p_error_code IS NULL OR p_error_code !~ '^FK_[A-Z_]+$' THEN
    RAISE EXCEPTION 'FK_INVALID_INPUT: error_code must match FK_[A-Z_]+ format';
  END IF;
...
### 13.17 Concurrent safety — executable two-session companion tests

These tests require two concurrent psql sessions. They are provided as paired scripts (`V2_race_test_A.sql` / `V2_race_test_B.sql`) where step numbers must be executed in the documented interleaving. Each test ends with ROLLBACK in both sessions to leave the DB clean.

**Race Test RT-1: reservation vs. deletion (two sub-tests)**

RT-1 covers both orderings of the reservation/deletion race. Both sub-tests use an automated two-session harness with blocking assertions via `pg_advisory_lock`, a 10-second timeout guard, exit codes (0 = pass, 1 = fail), and fixture cleanup in a `finally` block. Scripts are `V2_race_test_RT1a_A.sh` / `V2_race_test_RT1a_B.sh` and `V2_race_test_RT1b_A.sh` / `V2_race_test_RT1b_B.sh`.

**RT-1a — deletion wins the challenge lock first → reservation fails**
```
Fixture:     INSERT pending challenge (challenge_id=X, uploader_id=U) in both sessions' view.
             Both sessions connect; coordinator signals "ready" via pg_advisory_lock(1001).

Session B:   BEGIN;
Session B:   SELECT pg_advisory_xact_lock(1001);          -- signals Session A it holds lock
Session B:   [acquires challenge row FOR UPDATE internally via prepare_account_deletion_wrapper]
             [inserts database_prepared record; deletion now in progress for uploader U]
  → Advisory signal released; Session B holds the transaction open.

Session A:   BEGIN;
Session A:   SELECT reserve_upload_session(challenge_id:=X, uploader_id:=U, ...);
             -- Blocks on challenge FOR UPDATE held by Session B.
             -- Script asserts pg_locks shows the block within 2 seconds.

Session B:   COMMIT;   -- releases challenge lock; deletion committed

Session A:   -- Unblocks; finds database_prepared record for uploader U → FK_FORBIDDEN.
Session A:   ROLLBACK;

Assert:      Session A exit code = 1 and stderr contains 'FK_FORBIDDEN'.
             No pending upload_sessions row for challenge X.
Cleanup:     DELETE deletion_log WHERE profile_id = U; DELETE challenge WHERE id = X.
```

**RT-1b — reservation commits first → deletion quiesces the committed session**
```
Fixture:     INSERT pending challenge (challenge_id=X, uploader_id=U).

Session A:   BEGIN;
Session A:   SELECT reserve_upload_session(challenge_id:=X, uploader_id:=U, ...);  -- succeeds
Session A:   COMMIT;   -- session row now committed as 'pending'

Session B:   BEGIN;
Session B:   SELECT prepare_account_deletion_wrapper(p_uploader_id:=U);
             -- Acquires challenge FOR UPDATE; no competing lock; proceeds.
             -- quiesce_upload_sessions_for_deletion transitions pending session → failed.
Session B:   COMMIT;

Assert:      upload_sessions row for challenge X has status = 'failed'.
             quiesce_upload_sessions_for_deletion returned the session in its result set.
...
```
Session A:  BEGIN;
Session A:  SELECT session_id FROM private.upload_sessions WHERE status='sanitized' LIMIT 1 FOR UPDATE;
            -- holds session row lock (simulating finalize_upload_session holding lock)
Session B:  BEGIN;
Session B:  SELECT * FROM claim_cleanup_sessions('worker-B');
            -- SKIP LOCKED: the locked row is skipped; other eligible rows claimed
Session A:  ROLLBACK;
Session B:  ROLLBACK;
```
Expected: Session B does not block on Session A's locked row; returns without it.

**Race Test RT-3: concurrent reservation (unique index)**

Automated two-session harness: `V2_race_test_RT3_A.sh` / `V2_race_test_RT3_B.sh`. Scripts coordinate via `pg_advisory_lock`, enforce a 10-second timeout, and clean up fixtures in a `finally` block. Exit code 0 = pass, 1 = fail.

```
Fixture:     INSERT pending challenge (challenge_id=X, uploader_id=U) visible to both sessions.

Session A:   BEGIN;
Session A:   SELECT reserve_upload_session(p_challenge_id:=X, p_uploader_id:=U, ...);  -- succeeds
Session A:   COMMIT;   -- IMPORTANT: Session A commits before Session B attempts.
             -- The unique index entry is now visible to other transactions.

Session B:   BEGIN;
Session B:   SELECT reserve_upload_session(p_challenge_id:=X, p_uploader_id:=U, ...);
             -- Unique index (upload_sessions_one_active_per_challenge) conflict.
             -- reserve_upload_session catches the unique_violation and raises FK_UPLOAD_IN_PROGRESS.
Session B:   ROLLBACK;

Assert:      Exactly one upload_sessions row for challenge X with status = 'pending'.
             Session B exit code = 1 and stderr contains 'FK_UPLOAD_IN_PROGRESS'.
Cleanup:     DELETE upload_sessions WHERE challenge_id = X;
             DELETE challenge WHERE id = X.
```
Expected: Exactly one reservation committed; the other gets `FK_UPLOAD_IN_PROGRESS`. No partial rows, no deadlock.

**Race Test RT-4: concurrent deletion-recovery claiming**
```
Session A:  BEGIN;
Session A:  SELECT * FROM claim_deletion_recovery_records('worker-A');
            -- holds FOR UPDATE SKIP LOCKED on target deletion_log row
Session B:  BEGIN;
Session B:  SELECT * FROM claim_deletion_recovery_records('worker-B');
            -- SKIP LOCKED: same row skipped; Session B gets no result for that row
Session A:  COMMIT;
Session B:  COMMIT;
```
Expected: Each claimed row is returned by exactly one session; no claim-token overwrite.
...
- [ ] `record_deletion_failure_wrapper` validates `FK_[A-Z_]+` format before calling V1 private function
