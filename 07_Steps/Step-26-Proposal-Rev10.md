...
**Supersedes:** Rev 9 (rejected — GPT review, 4 blockers)
...
| Blocker | Fix |
|---|---|
| 1 | Poster explicitly excluded from `investigation_members` snapshot in `launch_case()`. `submit_guess()` rejects poster at Step 1b. Poster stays outside scoring. New `is_case_poster_or_member` RLS helper added for Table Talk, progress, and revealed results access. Test: Table containing only the poster fails at launch. |
| 2 | `INSERT` on `guess_attempts` revoked from `authenticated`, `anon`, and `PUBLIC`. Only `forkensics_executor` retains it. Direct-insert test expects `SQLSTATE 42501`. Full function privilege matrix added (§16). |
| 3 | Both `cancel_case()` (in `launched` state) and `cancel_investigation()` reject when `clock_timestamp() >= cases.deadline_at`. Test: past-deadline launched case cannot be cancelled. |
| 4 | `report_content()` cancelled-case guard extended to all case-linked targets (`'case'`, `'comment'`, `'clue'`, `'media_object'`). For each, resolve and lock the owning case; reject if cancelled. Profile reports remain independent. Existing pending reports remain actionable. |
...
| Poster in investigation_members | No. Poster is excluded from every snapshot. Poster cannot submit a guess. Poster cannot score. |
| Poster investigation access | Poster sees Table Talk, progress, and revealed results via dedicated RLS policy (see §19). |
...
| New reports on cancelled cases | Rejected for all case-linked targets (`case`, `comment`, `clue`, `media_object`). Existing pending reports remain actionable. Profile reports are independent. |
...
The poster (`cases.poster_id`) is **never inserted** into `investigation_members`. This is enforced in `launch_case()` by filtering `gm.player_id != poster_id` during the snapshot INSERT. The poster cannot score, cannot be excluded, and does not appear in the member roster.
...
--   GRANT INSERT ON guess_attempts TO forkensics_executor;  -- only path is submit_guess()
--   GRANT SELECT ON guess_attempts TO forkensics_rls_helper, authenticated;
```
...
| `launched` | Yes — all case-linked target types | Actionable | Yes |
| `locked` | Yes | Actionable | Yes |
| `revealed` | Yes | Actionable | Yes |
| `retired` | Yes | Actionable | Yes |
| `cancelled` | **No** — `report_content()` rejects new reports for `target_type IN ('case','comment','clue','media_object')` where the owning case is cancelled | **Still actionable** — `get_moderation_queue`, `action_report`, `dismiss_report`, `remove_content`, `remove_media` work regardless of case state. | Still executable |

Profile reports (`target_type = 'profile'`) are independent of any case and are always accepted.

### `report_content()` — cancelled case guard (V4, all case-linked targets)

After validating the caller and locking the target row, for each case-linked `target_type`, resolve and lock the owning case, then reject if cancelled:

```sql
-- Resolve owning case for each target type, then lock it
IF p_target_type = 'case' THEN
  SELECT id, state INTO v_case FROM public.cases WHERE id = p_target_id FOR SHARE;
ELSIF p_target_type = 'comment' THEN
  SELECT c.id, c.state INTO v_case
  FROM public.cases c JOIN public.comments cm ON cm.case_id = c.id
  WHERE cm.id = p_target_id FOR SHARE OF c;
ELSIF p_target_type = 'clue' THEN
  SELECT c.id, c.state INTO v_case
  FROM public.cases c JOIN public.clues cl ON cl.case_id = c.id
  WHERE cl.id = p_target_id FOR SHARE OF c;
ELSIF p_target_type = 'media_object' THEN
  SELECT c.id, c.state INTO v_case
  FROM public.cases c WHERE c.media_object_id = p_target_id FOR SHARE;
  -- If no case owns this media_object, v_case is NULL and the guard is skipped
END IF;

IF v_case.state = 'cancelled' THEN
  RAISE EXCEPTION 'FK_INVALID_INPUT: cannot report content belonging to a cancelled case';
END IF;
-- profile target_type: v_case is never set; guard is skipped.
```

Pending reports created before cancellation remain in the queue and are fully actionable via all moderation functions.
...
-- Poster access: can see Table Talk, progress, revealed results for any investigation
-- on their case, even though they are not in investigation_members.
...
| `public.report_content(text, uuid, text, text)` | CREATE OR REPLACE; `'challenge' → 'case'`; all-targets cancelled-case guard (§13) |
...
### Indexes — see §15 table in Rev 9 (unchanged)

---

## 16. Function Privilege Matrix

All functions are owned by either `forkensics_executor` (mutation path) or `forkensics_rls_helper` (read/helper path). `PUBLIC` EXECUTE is revoked from all functions via `REVOKE ALL ON FUNCTION ... FROM PUBLIC` before selective grants.
...
| `public.remove_content(text, uuid, uuid, uuid, text)` | `authenticated` |
| `public.remove_media(uuid, uuid, uuid, text)` | `authenticated` |
| `public.approve_photo(uuid, uuid, text)` | `service_role` only — NOT `authenticated` |
| `public.reject_photo(uuid, uuid, text)` | `service_role` only |
| `public.claim_moderation_media_cleanup(int)` | `service_role` only |

### Scheduler / service-role only (revoked from `authenticated`)

| Function | Granted to |
|---|---|
| `public.lock_case(uuid)` | `service_role` |
| `private.reveal_case_service(uuid)` | `service_role` |
| `public.get_media_serve_authorization(uuid, uuid)` | `service_role` |
| `public.get_moderation_queue()` | `service_role` |
| `public.get_pending_review_media(uuid)` | `service_role` |
| `public.get_reported_media(uuid)` | `service_role` |

### Internal / executor-only (no client grant)

| Function | Notes |
|---|---|
| `private.do_reveal_impl_v3(uuid, uuid)` | SECURITY DEFINER; called by reveal_case and reveal_case_service |
| `private.prepare_account_deletion(uuid)` | SECURITY DEFINER; called only via wrapper |
| `private.mark_auth_deleted(uuid)` | Internal deletion pipeline |
| `private.mark_storage_cleaned(uuid)` | Internal deletion pipeline |
| `private.record_deletion_failure(uuid, text)` | Internal deletion pipeline |
| `private.get_storage_keys_for_deletion(uuid)` | Internal deletion pipeline |
| `private.reveal_case_service(uuid)` | `service_role` only; not `authenticated` |
| `private.can_view_case(uuid)` | SECURITY DEFINER; called from policies |
| `private.can_viewer_access_case(uuid, uuid)` | SECURITY DEFINER; called from get_media_serve_authorization |
| `private.has_block_with_poster(uuid)` | SECURITY DEFINER; called from policies |

### RLS helpers (no direct client grant; called from policies)

All functions in `private` schema: `is_case_member`, `is_investigation_member`, `is_case_poster`, `is_case_revealed`, `is_investigation_eligible`, `is_case_poster_for_investigation`, `caller_has_guessed`, `auth_uid`, `normalize_answer`, `is_group_member`, `is_group_member_with`.

### Table INSERT privileges

| Table | INSERT granted to |
|---|---|
| `guess_attempts` | `forkensics_executor` ONLY. `REVOKE INSERT ON guess_attempts FROM authenticated, anon, PUBLIC` in Phase 20. |
| All other public tables | Per existing V1/V2/V3 grants; unchanged unless noted. |
...
Caller holds FOR UPDATE on cases. Does NOT update `cases.state`. Scores only players with `eligibility_status = 'eligible'` in the specified investigation. Deleted players are excluded automatically.
...
IF state = 'launched' AND clock_timestamp() >= deadline_at THEN
  RAISE EXCEPTION
    'cancellation deadline has passed: case is pending scheduler lock (state still launched)';
END IF;
...
**Full contract with poster rejection and race-branched idempotency:**
...
Step 1b — Poster rejection (no DB read; case fetched in Step 4)
  -- Checked again at Step 4 after locking. Resolved here eagerly to fail fast.
  IF EXISTS (SELECT 1 FROM public.cases WHERE id=p_case_id AND poster_id=auth_uid())
    → EXCEPTION 'FK_FORBIDDEN: poster cannot submit a guess on their own case';
...
Step 3 — Idempotency lookup (BEFORE state/deadline/eligibility checks)
  ...
  IF v_existing.id IS NOT NULL THEN
    ...
    ELSE  -- 'where'
      ...
      END IF;
    END IF;
    RETURN v_existing;  -- Identical payload; idempotent replay
  END IF;
...
Step 4 — Case state, deadline, and poster check (locked)
  SELECT state, deadline_at, poster_id FROM cases WHERE id=p_case_id FOR SHARE;
  IF NOT FOUND → EXCEPTION 'case not found';
  IF v_case.poster_id = auth_uid() → EXCEPTION 'FK_FORBIDDEN: poster cannot submit a guess';
...
Step 7 — INSERT
  ...
  -- Trigger set_guess_receipt_fields fires.
  -- UNIQUE constraints enforced; concurrent duplicate → catch → reload → re-apply Step 3
```
...
Step 11 — Snapshot investigation members (poster excluded)
  For each group:
    ...
    -- If zero detectives remain after excluding poster → RAISE 'group has no eligible detectives';

RETURN QUERY SELECT investigation_id, group_id FROM investigations WHERE case_id=p_case_id;
```

**Test: Table containing only the poster.** If all eligible members are excluded or only the poster is in the group, zero detectives remain after filtering → `launch_case()` raises `FK_INVALID_INPUT` or equivalent. The case state is rolled back (no investigation created). Acceptance criterion: posting a case to a solo-poster group fails at launch.

---

## 19. RLS — Guess Visibility and Poster Investigation Access
...
### Poster access to investigation-scoped tables

The poster is not in `investigation_members` but needs read access to Table Talk (comments, reactions), progress (score_runs, score_events), and revealed results (guess_judgments, investigation_members roster) for each investigation on their case.

Each investigation-scoped table gets an additional poster-access policy:
...
--   reactions, score_runs, score_events, current_score_events (view),
--   guess_judgments, investigation_members (roster)
```

The poster reads these tables but cannot INSERT into them (no `investigation_members` row means investigation-level INSERT policies don't match either).
...
Step 3 — Transfer or archive owned groups
  For each group where gm.player_id=p_profile_id, role='owner', not archived:
    Successor: is_active=true, onboarding_complete=true, is_suspended=false,
...
Phase 1 — Rename challenges → cases
  ...
  1b. DROP CONSTRAINT challenges_state_check; ADD CONSTRAINT cases_state_check (temp, includes 'active')
...
Phase 3 — Moderation target type migration (two-step constraint replace)
  ...
  3b. DROP ma_target_type_check; ADD ma_target_type_check_tmp (both 'challenge' + 'case')
  ...
  3e. DROP cr_target_type_check_tmp; ADD cr_target_type_check (case only)
  3f. DROP ma_target_type_check_tmp; ADD ma_target_type_check (case only)
...
Phase 6 — Rename challenge_id on dependent tables
  6a. guess_attempts: RENAME → case_id; ADD idempotency_key nullable→backfill→NOT NULL;
      ADD UNIQUE (case_id, player_id, race); ADD UNIQUE (case_id, player_id, idempotency_key)
  6b. exclusion_events: RENAME → case_id; ADD investigation_id nullable→backfill→NOT NULL;
      DROP old UNIQUE; ADD UNIQUE (investigation_id, player_id); cross-record trigger
  6c. clues: RENAME → case_id
  6d. comments: RENAME → case_id; ADD investigation_id nullable→backfill→NOT NULL; cross-record trigger
  ...
  6g. score_runs: RENAME → case_id; ADD investigation_id nullable→backfill→NOT NULL;
      DROP old UNIQUE; ADD UNIQUE (investigation_id, revision_number); cross-record trigger
  ...
  6j. reactions: ADD investigation_id nullable; backfill; NOT NULL; DROP challenge_id;
      ADD FK; replace UNIQUE (investigation_id, player_id, emoji)
...
Phase 8 — Upload sessions
  ...
  UPDATE original_storage_path + display_storage_path (replace 'challenges/' → 'cases/')

Phase 9 — RLS helper rebuilds
  DROP is_challenge_group_member; CREATE is_case_member + is_investigation_member +
    is_case_poster_for_investigation;
...
Phase 10 — Operational function rebuilds
  private.prepare_account_deletion: full rebuild per §21
  ...
  public.cancel_case: DROP cancel_challenge + CREATE (with deadline guard)
  public.cancel_investigation: CREATE (new, with deadline guard)
...
Phase 11 — Step 24.1 function updates (all per §15 + §13)

Phase 12 — V2 function updates (per §14)

Phase 13 — Trigger rebuilds (per §15)

Phase 14 — Index rebuilds (per §15)

Phase 15 — RLS policies (per §15 + §19)
  — includes poster-access policies on investigation-scoped tables
...
Phase 16 — State conversion: active → launched
  ...
  16d. DROP cases_state_check (temp); ADD cases_state_check (final, no 'active')

Phase 17 — Partial index update
  DROP one_active_challenge_per_poster;
  CREATE UNIQUE INDEX one_active_case_per_poster ON cases (poster_id)
    WHERE state IN ('draft','ready','launched','locked')

Phase 18 — Remove group_id from cases
  pg_depend audit; DROP CONSTRAINT challenges_group_id_fkey;
  DROP INDEX idx_challenges_group_id; DROP COLUMN group_id

Phase 19 — New service functions
  launch_case (per §18); submit_guess (per §17)

Phase 20 — Grants, ownership, privilege hardening, completion marker
  -- Revoke direct INSERT on guess_attempts
  ...
  -- All other table grants per §16 privilege matrix
  ...
  -- Function ownership and EXECUTE per §16 privilege matrix
  All new/renamed functions: OWNER TO forkensics_executor (or forkensics_rls_helper)
  REVOKE ALL ON FUNCTION ... FROM PUBLIC before selective grants
...
- `investigation_members`: no row where `player_id = cases.poster_id` (verified by COUNT query after migration)

**Privilege hardening:**
- `REVOKE INSERT ON guess_attempts FROM authenticated` confirmed in pg_catalog
- Direct `INSERT INTO guess_attempts` by `authenticated` user → `SQLSTATE 42501`
- `forkensics_executor` INSERT succeeds
- Function privilege matrix (§16): each function has exactly the grants specified; no extra grants to `authenticated` or `PUBLIC`

**Moderation:**
- `content_reports` and `moderation_actions`: no `'challenge'` rows post-migration; final constraints reject `'challenge'`
- New report on cancelled case (target_type='case') → rejected
- New report on comment/clue/media_object belonging to cancelled case → rejected
- New report on profile → accepted regardless of case state
- Existing pending report on cancelled case target → still in queue and actionable
- `action_report`, `dismiss_report`, `remove_content`, `remove_media` succeed on cancelled-case targets

**Core functions:**
- `lock_case`: rejects before `deadline_at`; rejects non-`launched` state
...
- `cancel_case`: poster only; no group owner authority; cancels case + all active investigations; past-deadline `launched` case → rejected
- `cancel_investigation`: rejected when `cases.state != 'launched'`; past-deadline `launched` case → rejected; suspended caller rejected; case state unchanged; test: cancel after reveal fails; test: cancel with past deadline fails
- All-investigations-cancelled: case stays in current state; reveal scores nothing; case still → `revealed`

**Poster access:**
- Poster not in `investigation_members` (confirmed by SELECT)
- `submit_guess()` → `FK_FORBIDDEN` when `auth_uid() = poster_id`
- `launch_case()`: group containing only the poster → launch fails with zero detectives error
- Poster can SELECT from comments, reactions, score_runs, score_events, guess_judgments, investigation_members on their case (via `is_case_poster_for_investigation` policy)
- Poster cannot INSERT into investigation-scoped tables

**`launch_case()`:**
- `p_actor_id != auth_uid()` → `FK_FORBIDDEN`
- Non-poster rejected at Step 4
- Deletion guard works
- Poster excluded from snapshot; at least one non-poster eligible detective required per investigation

**`submit_guess()`:**
- Poster → `FK_FORBIDDEN` (Step 1b and Step 4 both checked)
- Suspended caller → rejected
- Bilateral block with poster → rejected
- `clock_timestamp() >= deadline_at` → rejected
- Idempotency: same key + identical payload (race-branched) → existing row returned
- Same key + different race → `FK_CONFLICT`; same key + different answer → `FK_CONFLICT`
- Concurrency: two sessions submit identical guess simultaneously → exactly one row; both return correctly

**RLS:**
- Revealed-guess: excluded/removed viewer cannot see — eligibility required
- Cross-Table isolation: Table A cannot see Table B-only guesses, comments, reactions, members, scores

**Account deletion:**
- `eligibility_status = 'eligible'` → `'account_deleted'` for ALL investigations regardless of case state
- `apply_correction()` on revealed case after player deleted → player gets 0 points
- Suspended successor skipped in group transfer
