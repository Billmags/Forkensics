...
**Supersedes:** Rev 14 (rejected — GPT review, 1 blocker)
...
| Blocker | Fix |
|---|---|
| 1 | `removed_at` does not exist on `comments` or `clues`. Column is `moderator_removed_at` (established in Step 24.1). Both branches updated. Race-test expectations corrected: report-wins means the report is subsequently `actioned` and linked to the removal action (no pending report remains after removal); removal-wins means `FK_NOT_FOUND` is raised and no report is created. All assertions reference `moderator_removed_at`. |
...
| Removed-content column name | `moderator_removed_at` (Step 24.1). Not `removed_at`. |
...
Governing rule: target row is locked FOR UPDATE first, then the owning case is locked FOR UPDATE. This matches `remove_content()`'s established lock order and prevents deadlocks and stranded reports.
...
IF p_target_type = 'case' THEN
  -- The case is both the target and the owning case; one FOR UPDATE satisfies both.
...
ELSIF p_target_type = 'comment' THEN
  ...
  SELECT id, case_id, moderator_removed_at INTO v_comment
  FROM public.comments WHERE id = p_target_id
  FOR UPDATE;
  -- Step 2: validate it exists and has not been moderator-removed
  IF NOT FOUND OR v_comment.moderator_removed_at IS NOT NULL THEN
    RAISE EXCEPTION 'FK_NOT_FOUND';
  END IF;
  -- Step 3: lock the owning case FOR UPDATE (separate query, after target locked)
  SELECT id, state INTO v_case
...
ELSIF p_target_type = 'clue' THEN
  ...
  SELECT id, case_id, moderator_removed_at INTO v_clue
...
ELSIF p_target_type = 'media_object' THEN
  ...
  FOR UPDATE;
  -- Step 6: missing or not ready → FK_NOT_FOUND (uniform, no information leak)
...
-- media_object handled Steps 4a–6 inline above; safe no-op for that path.
IF v_case_id IS NOT NULL AND v_case.state = 'cancelled' THEN
  RAISE EXCEPTION 'FK_INVALID_INPUT: cannot report content belonging to a cancelled case';
END IF;

-- Continue: insert content_reports row, update report count, etc.
```
...
This order matches `remove_content()`'s established lock order, preventing deadlocks between the two functions.
...
-- can_view_case: RLS policies (authenticated), service path (service_role),
-- and SECURITY DEFINER functions executing as forkensics_executor
...
-- can_viewer_access_case: outer caller is service_role;
-- inner execution is as forkensics_executor (SECURITY DEFINER owner)
...
Eight steps unchanged. Step 2 marks ALL `eligibility_status='eligible'` rows `'account_deleted'` regardless of case state; exclusion_events created only for active investigations in launched/locked cases. Step 5 sets `profiles.is_active = false`.
...
**Phase 20** — Grants, privilege hardening, completion marker (full list per §17).
...
| Comment with `moderator_removed_at IS NOT NULL` | `FK_NOT_FOUND` |
| Clue with `moderator_removed_at IS NOT NULL` | `FK_NOT_FOUND` |
| Existing pending report on cancelled target | Still actionable |

### Two-session race tests — comment and clue

**Report-wins scenario (comment and clue equivalently):**
1. Session 1 calls `report_content('comment', comment_id, ...)` — acquires comment FOR UPDATE, inserts report, commits.
2. Session 2 calls `remove_content('comment', comment_id, ...)` — acquires comment FOR UPDATE (after Session 1 commits), sets `moderator_removed_at`, actions all pending reports against this target (linking each to the removal action), commits.
3. Expected outcome: the report created in step 1 has `status = 'actioned'` and is linked to the removal action. No pending report remains. Assertions:
   - `SELECT status FROM content_reports WHERE target_id = comment_id AND status = 'pending'` → 0 rows
   - `SELECT moderator_removed_at FROM comments WHERE id = comment_id` → NOT NULL

**Removal-wins scenario (comment and clue equivalently):**
1. Session 1 calls `remove_content('comment', comment_id, ...)` — acquires comment FOR UPDATE, sets `moderator_removed_at`, commits.
2. Session 2 calls `report_content('comment', comment_id, ...)` — acquires comment FOR UPDATE (after Session 1 commits), reads `moderator_removed_at IS NOT NULL` → raises `FK_NOT_FOUND`, rolls back.
3. Expected outcome: no report is created for the removed comment. Assertions:
   - `SELECT count(*) FROM content_reports WHERE target_id = comment_id AND created_at > removal_time` → 0
   - `SELECT moderator_removed_at FROM comments WHERE id = comment_id` → NOT NULL
...
- Poster SELECT `current_score_events` via view → receives scores
- Poster INSERT into `investigation_members` → rejected
- Poster INSERT into `score_events` → rejected

### Core function tests (unchanged from Rev 14)
...
- `submit_guess()` poster → `FK_FORBIDDEN`; suspended → rejected; past-deadline → rejected
