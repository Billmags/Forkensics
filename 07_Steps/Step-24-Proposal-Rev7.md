# Step 24 Proposal — Rev 7 — Edge Function Architecture and Contracts
...
**Changes from Rev 6:**
1. V1 deletion-function schema corrected: `mark_auth_deleted` and `mark_storage_cleaned` are in the `private` schema and cannot be called through PostgREST. Explicit public wrappers added: `public.mark_auth_deleted_wrapper(uuid)` and `public.mark_storage_cleaned_wrapper(uuid)`. All deletion contracts updated to call these wrappers.
2. Upload concurrency hardened with three additions: (a) a partial unique index on `private.upload_sessions(challenge_id)` where status is `pending`, `processing`, or `sanitized` — enforced at the database constraint level, not only by application logic; (b) a V2 trigger on `public.challenges` that rejects transition to `active` while any active upload session exists for that challenge; (c) `finalize_upload_session` locks the challenge row and verifies state is still `draft` before committing, preventing finalization from replacing a photo on a live challenge. Concurrency test added for activation vs. finalization.
3. Completed-session idempotency fixed: `media_object_id` and `replaced_media_object_id` columns added to `private.upload_sessions`. `finalize_upload_session` saves both atomically. `resolve_upload_session` returns them. `upload-complete` uses these values for the completed-session idempotency response.
4. Account deletion race eliminated with a quiescing step: `fail_all_pending_sessions_for_user` replaced by `public.quiesce_upload_sessions_for_deletion(user_id)`, which atomically stops `pending` and `sanitized` sessions and returns their paths, but returns a blocking signal for any `processing` session whose lease is still valid. `account-delete-complete` returns `409 FK_UPLOAD_IN_PROGRESS` (with `retry_after`) if a blocking session exists and does not proceed with storage deletion. `upload-complete` verifies its processing lease before writing the display object (new step 5.5 using `public.check_upload_session_lease`); if the lease is gone, it deletes the original and aborts. If `advance_upload_session_sanitized` is rejected for any reason, `upload-complete` deletes the display object before returning. Both scans of the deletion-recovery-worker use the same quiescing pattern. `complete_deletion_recovery` verifies the supplied `scan_type` matches the stored `scan_type` in the claim row. Both infrastructure tables are placed in the `private` schema in Step 25.
...
- `complete`: DB finalization committed; all DB records set; terminal
...
Acquires a row-level lock on the challenge (`SELECT ... FOR UPDATE`) and verifies `challenges.state = 'draft'` before committing. If the challenge is not `draft` → raises `FK_WRONG_STATE`. Atomically in a single transaction: inserts `public.media_objects` (`status = 'ready'`); inserts `private.media_storage_keys` (using both storage paths from the session row); if `challenges.media_object_id` is null: sets it to the new `media_object_id`; if already set: atomically updates to new `media_object_id` and sets prior `media_objects.status = 'superseded'`; saves `media_object_id` and `replaced_media_object_id` in the session row; transitions session to `complete`; updates `status_changed_at`. Idempotent: if already `complete`, returns the stored IDs without modification.

`public.fail_upload_session(session_id uuid, error_code text)`
→ void
Transitions to `failed`; records `failed_reason`; updates `status_changed_at`. Idempotent if already `failed`.
...
3. Selects all `expired` and `failed` sessions, plus `sanitized` sessions where `processing_lease_expires_at + interval '60 minutes' < now()` — where no claim exists or `cleanup_claim_expires_at < now()`.
4. Issues new `cleanup_claim_token`; sets `cleanup_claimed_at`, `cleanup_claim_expires_at`.
5. Returns claimed rows with tokens. Never returns `complete` or `cleaned` sessions.

`public.mark_session_cleaned(session_id uuid, claim_token uuid)`
→ void
Verifies `cleanup_claim_token = claim_token` and `cleanup_claim_expires_at > now()`. If invalid or expired → raises an error; does not transition. If valid: transitions to `cleaned`; sets `cleanup_completed_at`, `status_changed_at`. Idempotent if already `cleaned`.
...
Wrapper for `private.get_storage_keys_for_deletion(user_id)`. Returns one distinct row per physical storage key for established `media_objects`. Does not include upload session storage paths.
...
- `account-delete-complete` storage or quiesce-blocking failure → user can still authenticate and retry; no storage abandoned
...
8. Call `public.advance_upload_session_sanitized(session_id)`. If this raises for any reason (including the session having been failed by a concurrent deletion quiesce): delete display object (just written at `session.display_storage_path`); return `500 FK_INTERNAL`. Do not retry — the session is no longer in `processing` state. The display object at `session.display_storage_path` is a deterministic path; the cleanup worker will delete it on its next run.

**Finalization (step 8 of happy path, or entered from `sanitized` branch):**

9. Call `public.finalize_upload_session(session_id)`. Returns `(media_object_id, replaced_media_object_id)`. Acquires row-level lock on challenge; verifies state is still `draft`. If challenge not `draft` → return `409 FK_WRONG_STATE`. Atomically: inserts `media_objects`, `media_storage_keys`; sets or swaps `challenges.media_object_id`; saves result IDs in session row; transitions to `complete`. If already `complete`, returns stored IDs. If finalization fails for other reasons: session stays `sanitized`; return `500 FK_INTERNAL`. Client retries with same token — enters `sanitized` branch.
...
**Purpose:** Initiate or complete account deletion. Quiesces upload sessions, gathers and deletes all storage, deletes `auth.users`, finalizes deletion record.
...
2. Call `public.quiesce_upload_sessions_for_deletion(user_id)` (administrative client). Returns rows of stopped sessions and any blocking sessions.
3. If any returned row has `blocking_lease_expires_at IS NOT NULL` (active processing session):
   ...
   - **Stop here.** The stopped `pending` and `sanitized` session storage will be collected on the client's next retry call (which will call quiesce again; stopped sessions will now be `failed` and not returned by quiesce, but their storage paths may still need deletion — see step 3a below).
   - Note: on the next retry after the blocking lease expires, step 2 is called again. The previously-stopped sessions are now `failed`; quiesce only stops currently active sessions. Their storage is collected in step 3a.
3a. Delete storage objects returned by quiesce for stopped sessions (rows with `blocking_lease_expires_at = NULL`). Attempt `original_storage_path` and `display_storage_path` as applicable per `prior_status`. Absent objects are no-op successes. Track failures.
4. Call `public.get_deletion_storage_keys(user_id)` (administrative client). Returns established media keys.
5. Delete all established media storage objects from `game-media`. Track failures.
6. If any storage deletion from step 3a or step 5 fails:
   ...
   - **Stop here.**
7. Delete the `auth.users` row (Admin API, administrative client). If fails → `500 FK_INTERNAL`. User can retry.
8. Call `public.mark_auth_deleted_wrapper(user_id)`. If fails: JWT is now void; client cannot retry. Log for recovery worker. Return `500 FK_INTERNAL`.
9. Call `public.mark_storage_cleaned_wrapper(user_id)`. Transitions to `complete`. If fails: same. Log for recovery worker. Return `500 FK_INTERNAL`.
10. Return `200 {"status":"complete"}`.

**Retry note on stopped sessions from a prior blocked attempt:** On the next call after a `409 FK_UPLOAD_IN_PROGRESS`, step 2 (quiesce) will not return the previously-stopped `failed` sessions — those are past the quiesce function's scope. Their storage paths are deterministic (stored in `private.upload_sessions`) and will be cleaned by the upload-cleanup-worker. Deletion completion is not blocked on cleanup-worker work for these sessions; their storage is effectively orphaned from the established-media tracking perspective and handled separately. If privacy requires guaranteeing all paths are deleted before completion, the deletion function may instead also call `get_cleanup_sessions` to fetch failed session paths — this is a Step 25 decision.

**Logging:** `user_id`, `quiesced_pending_count`, `quiesced_sanitized_count`, `blocking_session_count`, `established_key_count`, `storage_objects_attempted`, `deleted_count`, `failed_count`, deletion record state at each step. NOT storage paths, key values, or file content.
...
1. Call `public.claim_cleanup_sessions(worker_id, '15 minutes')`. Atomically transitions stale sessions and claims work. Returns sessions with `cleanup_claim_token`.
2. For each:
   a. Attempt to delete `original_storage_path` (for `expired`, `failed` sessions — may or may not exist). Absent = no-op.
   b. Attempt to delete `display_storage_path` (for `sanitized`, `failed` sessions — known from creation). Absent = no-op.
   c. If all applicable deletions succeeded: call `public.mark_session_cleaned(session_id, cleanup_claim_token)`. Verifies token before transitioning.
...
2. For each claimed record:
   ...
   **If `scan_type = 'database_prepared'`:**
   ...
   d. Delete storage from quiesced sessions (paths with `blocking_lease_expires_at = NULL`). Absent = no-op.
   e. Call `public.get_deletion_storage_keys(user_id)`. Delete all established media storage. Absent = no-op.
   f. If any storage deletion failed → `public.fail_deletion_recovery(user_id, claim_token, 'FK_PROCESSING_FAILED')`; leave in `database_prepared`; retry next run.
   g. If all storage deleted → `public.complete_deletion_recovery(user_id, claim_token, 'database_prepared')`. Calls `mark_auth_deleted_wrapper` then `mark_storage_cleaned_wrapper` → `complete`.
   ...
   **If `scan_type = 'auth_deleted'`:**
   ...
   c. Delete storage from quiesced sessions and established media keys (same as steps d–e above).
   d. If any storage deletion failed → `public.fail_deletion_recovery(user_id, claim_token, 'FK_PROCESSING_FAILED')`; retry next run.
   e. If all deleted → `public.complete_deletion_recovery(user_id, claim_token, 'auth_deleted')`. Calls `mark_storage_cleaned_wrapper` → `complete`.
...
- `claim_cleanup_sessions` issues `cleanup_claim_token` stored in session row
- `mark_session_cleaned` with wrong token → error; session not transitioned
- `mark_session_cleaned` with expired claim → error; session not transitioned
- Two concurrent workers: SKIP LOCKED; each claims disjoint sessions
- Unexpired active claim: not re-claimed by second worker
- Expired claim: re-claimable on next run
- Stale `pending` (past `expires_at + 60s`) → `expired` by `claim_cleanup_sessions`
- Stale `processing` (past `processing_lease_expires_at`) → `failed` by `claim_cleanup_sessions`
- Abandoned `sanitized` (past `processing_lease_expires_at + 60 min`) → claimed for cleanup
- `complete` sessions → never returned by `claim_cleanup_sessions`
...
- `account-delete-complete` with `pending` session → quiesce fails it; `original_storage_path` deleted
- `account-delete-complete` with `sanitized` session → quiesce fails it; `display_storage_path` deleted
- Storage failure after quiesce → `422`; auth not deleted; user can retry
- Recovery worker: same quiescing behavior in both scans
...
- [ ] `finalize_upload_session` agreed: locks challenge row; verifies `draft` state before committing; handles photo replacement atomically
