# Step 25 Proposal — Rev 7 — V2 Migration: Upload Session Infrastructure
...
**Changes from Rev 6 (addressing 1 Codex blocker):**
1. RT-2 control assertion corrected: `prior_status = 'sanitized'` replaced with the actual `claim_cleanup_sessions` return columns (`session_id`, `status = 'failed'`, non-NULL `cleanup_claim_token`). Assert now also requires a direct `SELECT` on `private.upload_sessions` confirming stored `status = 'failed'`, and a pre-control direct `SELECT` confirming row S was still `'sanitized'` immediately before the control call — together proving the first exclusion was due to the lock, not ineligibility.

**Changes from Rev 5 (addressing 3 Codex blockers — preserved from Rev 6):**
1. RT-1a: replaced NOTIFY (which delivers only on commit, so cannot signal while Session B holds the challenge lock) with a harness-owned advisory lock. Harness pre-acquires `pg_advisory_lock(1001)` from Connection C; Session B calls `pg_advisory_xact_lock(1001)` after prepare, blocking until harness releases; harness detects B blocked, launches A, confirms A blocked, then releases 1001; B unblocks and commits.
2. RT-1b: converted from sequential to a true race. Session A reserves but holds the tx open (blocked on harness advisory lock 2001 using same pattern); harness launches B; B attempts prepare and blocks on challenge lock; harness releases 2001; A commits; B unblocks, completes prepare, calls quiesce explicitly, commits. Assert covers quiesce result set.
3. RT-2: fixture now sets `processing_lease_expires_at = now() - interval '70 minutes'` to make row S genuinely eligible for cleanup; control pass added — after A releases the lock, a fresh Session C runs claim_cleanup_sessions and confirms S is claimed and transitioned to 'failed', proving the first exclusion was caused by the lock, not ineligibility.
4. RT-3: corrected blocking description — Session B initially blocks on the challenge row FOR UPDATE held by Session A (not on the unique index); after A commits the challenge lock is released, B proceeds to INSERT, and then gets unique_violation → FK_UPLOAD_IN_PROGRESS.

**Changes from Rev 4 (addressing 5 Codex blockers — preserved from Rev 5):**
...
Coordination: harness pre-acquires an exclusive session-level advisory lock from a third
connection (C). Session B blocks on the same lock ID after prepare completes; harness
observes that block, launches A, waits for A to be blocked, then releases the lock. B
unblocks and commits. NOTIFY is NOT used — it delivers only on commit, so it cannot
signal while Session B's transaction is still open.

```
Fixture:     Snapshot-restore disposable DB. INSERT player U, draft challenge X (poster_id=U).

  [Harness/Connection C:  SELECT pg_advisory_lock(1001);
                          -- pre-acquires exclusive advisory lock; holds it open.]
...
Session B:   SELECT public.prepare_account_deletion_wrapper(p_user_id := U);
             ...
             -- Challenge row lock now held inside Session B's open transaction.
Session B:   SELECT pg_advisory_xact_lock(1001);
             -- BLOCKS: harness/C holds 1001 exclusively.
             -- This block signals the harness: "prepare is done, challenge lock is held."

  [Harness: detects Session B blocked on advisory lock 1001 in pg_stat_activity, max 10s]
  [Harness: launches Session A]

Session A:   BEGIN;
Session A:   SET statement_timeout = '10s';
Session A:   SELECT public.reserve_upload_session(p_challenge_id := X, p_uploader_id := U, ...);
             -- challenge FOR UPDATE → BLOCKS on challenge lock held by Session B.

  [Harness: detects Session A blocked (wait_event_type='Lock') in pg_stat_activity, max 5s]
  [Harness/Connection C:  SELECT pg_advisory_unlock(1001);  -- releases gate]

Session B:   -- Unblocks; acquires advisory lock 1001 (tx-level; released on commit).
Session B:   COMMIT;    -- releases challenge lock; cancellation committed.
...
**RT-1b — reservation wins challenge lock first → deletion blocks, then quiesces the committed session**

Coordination: same harness-owned advisory lock pattern. Harness pre-acquires lock 2001.
Session A blocks on 2001 after reserve (signaling reserve is done, challenge lock held).
Harness launches B. B blocks on the challenge lock. Harness detects B blocked, releases 2001.
A unblocks and commits. B unblocks, completes prepare, calls quiesce, commits.

```
Fixture:     Snapshot-restore disposable DB. INSERT player U, draft challenge X (poster_id=U).

  [Harness/Connection C:  SELECT pg_advisory_lock(2001);
                          -- pre-acquires exclusive advisory lock; holds it open.]

Session A:   BEGIN;
Session A:   SELECT public.reserve_upload_session(p_challenge_id := X, p_uploader_id := U, ...);
             -- INSERT pending upload_sessions row. Challenge FOR UPDATE held.
Session A:   SELECT pg_advisory_xact_lock(2001);
             -- BLOCKS: harness holds 2001. Signals: "reserve done, challenge lock held."

  [Harness: detects Session A blocked on advisory lock 2001 in pg_stat_activity, max 10s]
  [Harness: launches Session B]

Session B:   BEGIN;
Session B:   SET statement_timeout = '10s';
Session B:   SELECT public.prepare_account_deletion_wrapper(p_user_id := U);
             -- challenge FOR UPDATE → BLOCKS on challenge lock held by Session A.

  [Harness: detects Session B blocked (wait_event_type='Lock') in pg_stat_activity, max 5s]
  [Harness/Connection C:  SELECT pg_advisory_unlock(2001);  -- releases gate]

Session A:   -- Unblocks; acquires advisory lock 2001 (tx-level).
Session A:   COMMIT;  -- challenge lock released; upload_sessions row committed as 'pending'.

Session B:   -- Unblocks; prepare completes (cancels challenge, writes deletion_log).
Session B:   SELECT * FROM public.quiesce_upload_sessions_for_deletion(U);
             ...
             -- (prepare_account_deletion_wrapper does NOT call quiesce internally.)
Session B:   COMMIT;

Assert:      upload_sessions row for challenge X has status = 'failed'.
             Session B quiesce output contained exactly one row (the reserved session).
...
Fixture:     INSERT upload_sessions row with status='sanitized', session_id=S,
             processing_lease_expires_at = now() - interval '70 minutes'
             (satisfies the abandoned-sanitized eligibility threshold in claim_cleanup_sessions).
             Row S is therefore eligible for cleanup — the test must confirm exclusion is due
             to the lock, not ineligibility.
...
Session A:   SELECT pg_advisory_xact_lock(5001);    -- signals B: "lock held"

  [Harness: Session B waits for advisory lock 5001 in pg_locks, max 10s]

Session B:   BEGIN;
Session B:   SET statement_timeout = '10s';
Session B:   SELECT * FROM public.claim_cleanup_sessions('worker-B', ...);
             -- FOR UPDATE SKIP LOCKED: row S is locked by A → skipped; returns without it.
Session B:   COMMIT;

Assert (mid-test):
             Session B result set does NOT contain session_id = S.
             Session B returned without blocking.
             Row S still has status = 'sanitized' (not yet transitioned).

Session A:   ROLLBACK;   -- releases row lock on S.

  [Control pass: run claim_cleanup_sessions again with a fresh connection]

Session C:   BEGIN;
Session C:   SELECT * FROM public.claim_cleanup_sessions('worker-C', ...);
             -- Row S is now unlocked and eligible → claimed and transitioned to 'failed'.
Session C:   COMMIT;

Assert (control):
             Session C result set DOES contain session_id = S.
             The returned status = 'failed' (claim_cleanup_sessions transitioned it from sanitized
               before returning it; prior_status is not a return column).
             A non-NULL cleanup_claim_token was returned.
             A SELECT directly on private.upload_sessions confirms the stored row has status = 'failed'.
             Immediately before the control call, a direct SELECT confirmed row S still had
               status = 'sanitized' (proving the first cleanup pass left it untouched).
             Together these assertions prove the first exclusion was caused by the lock, not ineligibility.
...
Session B:   SELECT public.reserve_upload_session(p_challenge_id := X, p_uploader_id := U, ...);
             -- reserve_upload_session acquires challenge FOR UPDATE first.
             -- BLOCKS on the challenge row lock held by Session A (not on the unique index yet).
             -- Harness asserts Session B appears in pg_stat_activity with wait_event_type='Lock'
             --   within 2 seconds.

Session A:   COMMIT;    -- releases challenge row lock; Session B unblocks.

Session B:   -- Unblocks; challenge lock now available; proceeds to INSERT.
             -- INSERT conflicts with committed unique index entry from Session A.
             -- unique_violation → caught as FK_UPLOAD_IN_PROGRESS.
Session B:   ROLLBACK;
